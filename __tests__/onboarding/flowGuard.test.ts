...
describe('canAccessScreen', () => {
  ...
  describe('Customize', () => {
    it('allows when scenario is set', () => {
      expect(canAccessScreen('Customize', { slot: 1, scenario: { id: 'test' } })).toEqual({ allowed: true });
    });

    it('blocks when scenario is missing', () => {
      const result = canAccessScreen('Customize', { slot: 1 });
  ...
  describe('Perks', () => {
    ...
    it('blocks when scenario is missing', () => {
      const result = canAccessScreen('Perks', { slot: 1, firstName: 'Jane', lastName: 'Doe' });
      expect(result.allowed).toBe(false);
      expect(result.redirectTo).toBe('/(onboarding)/Scenarios');
    });

    it('blocks when firstName is empty', () => {
      const result = canAccessScreen('Perks', { slot: 1, scenario: { id: 'x' }, firstName: '', lastName: 'Doe' });
      expect(result.allowed).toBe(false);
      expect(result.redirectTo).toBe('/(onboarding)/Customize');
    });

    it('blocks when lastName is empty', () => {
      const result = canAccessScreen('Perks', { slot: 1, scenario: { id: 'x' }, firstName: 'Jane', lastName: '' });
      expect(result.allowed).toBe(false);
      expect(result.redirectTo).toBe('/(onboarding)/Customize');
    });
