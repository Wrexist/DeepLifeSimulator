...
### 2026-06-25 - Cold-container weekly audit: install deps before trusting the dynamic suite

- What went wrong: `npm run audit:weekly:full` reported a 🟠 high "Performance jest suite failed" on a fresh remote container. The failure was real-but-not-a-regression: `node_modules` was empty (the repo is cloned fresh per session), so `jest: not found`. The static suite passed; only the `--run-tests` backstop tripped, and it tripped on the environment, not the code.
- Pattern: the weekly-audit `--run-tests` backstop shells out to `jest`/`npm run test:performance`. On a cold clone with no `npm ci`, that surfaces as a domain FAIL that looks like a perf regression. Acting on it (or reporting it as blocking) would be a false alarm.
- Rule: at the start of any routine that runs the dynamic suite, confirm deps are installed (`ls node_modules/.bin/jest`) and run `npm ci` if not, THEN run `audit:weekly:full`. A jest "FAIL: suite failed / jest: not found" with an empty `node_modules` is an environment artifact — re-run after install before grading it.

### 2026-06-25 - New top-level state fields must be added to BOTH prestige preservation blocks

- What went wrong: `equippedCosmetics` (the player's chosen frame/theme) is account-level, but prestige rebuilds GameState from `initialGameState` and only the explicitly-listed fields survive. `legacyPass` (incl. `ownedCosmetics`) is preserved in both prestige blocks (`lib/prestige/prestigeExecution.ts:185-192`, `:373-378`), but `equippedCosmetics` was added later and never added to those blocks — so every prestige silently resets the equipped look to default (the cosmetic is still owned, just un-equipped). Tests missed it because `legacyPassPreservation.test.ts` only asserts `legacyPass` equality.
- Pattern: prestige preservation is an explicit allow-list, not a merge. Any new account-level/seasonal field added to GameState after the prestige code was written is silently dropped on prestige unless someone remembers to add it to BOTH blocks. The save/migration chain backfills it (so saves look fine) which masks the prestige-only loss.
- Rule: when adding an account-level field that must survive a life reset, add it to both preservation blocks in `prestigeExecution.ts` AND extend `legacyPassPreservation.test.ts` to seed and assert it. When auditing save/state, treat the prestige preservation allow-list as a drift surface distinct from initialState/migration/repair — a field can be fully wired into the save chain and still be wiped by prestige.
