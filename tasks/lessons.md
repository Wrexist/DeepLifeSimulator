...
### 2026-07-27 - Weekly audit: luxury "museum loan" is a permanent passive yield that escaped the catalog's "yield below upkeep" invariant (it lives in the VERBS system, not the `yield` field)

- Static + dynamic both CLEAN: `npm run audit:weekly` green across all 5 domains bar the chronic cosmetic 🟡 (47 `as GameState` casts in tests, pre-existing, factory-bypass hygiene). After `npm ci`, the dynamic backstop ran for real — moneyConservation.stress (4), the full luxury+save suite (175 tests incl. `luxuryHoldingsMigration`), performance (4), and `preflight:quick` type-check all pass. Save/state was solid: v24 `luxuryHoldings` + v25 `hasPilotLicense` are fully migration- AND repair-covered (`saveValidation.ts:485-499`) and the new `LuxuryHolding` fields are all optional/absent-means-default except `acquiredWeek` (backfilled), so no drift. Buy/sell/verbs/hosting all route cash through `applyMoneyDelta` inside the authoritative `setGameState(prev=>)` updater with the gate re-checked against `prev` — no double-grant, no cash-mirror desync, developable-property mint/remove is symmetric.
- The one real finding (MEDIUM, economy/exploit lens): the `museum_loan` verb (`lib/luxury/verbs.ts`) costs $0 and 0 energy, pays `MUSEUM_LOAN_WEEKLY_FEE = $4,000/wk` for `MUSEUM_LOAN_WEEKS = 12`, on a `cooldownWeeks: 12`. Loan duration == cooldown, so at week N+12 the loan expires AND the cooldown clears simultaneously → the diamond is re-loanable with a zero-week gap, i.e. it can be kept on public display permanently. Net cash while permanently loaned is +$4,000 − $200 upkeep = **+$3,800/wk from a $600k item** (≈158-week payback, then pure profit, plus +0.06%/wk appreciation). That directly contradicts the catalog's explicit design law (`catalog.ts`): "Yield is deliberately set BELOW each item's own upkeep — a trophy that paid for itself would stop being a trophy and start being an investment." Every PASSIVE catalog yield (racehorse 2.2k<4.5k, yacht 11k<20k, mega-yacht 85k<150k, team 130k<200k …) respects it; the museum loan is 20× upkeep and violates it.
- Pattern: the "yield below upkeep" invariant was only ever applied to the catalog `yield` field. The museum loan is *economically a passive yield* (permanent, ~zero-interaction — one tap per 12 weeks) but it's *implemented as a verb + a separate `getLoanIncome` credit in the weekly tick*, so it sailed past the author's own balance rule and past `audit-economy.cjs`, which audits constant ladders (APR ordering, tax monotonicity, miner prices) and has no notion of verb/loan income vs the owning item's upkeep. Not the H-3 printer class (fixed fee, single item, capped, manual-tap-gated, and the absolute magnitude is trivial next to the $100M–$1B endgame the diamond sits under) — which is why it's MEDIUM/filed, not blocking, and closest kin to the 2026-07-22 untaxed channel-membership "filed balance decision," not the 2026-07-24 gem farm. But it IS a self-financing luxury item, which the design says must never exist.
- Rule: any luxury income delivered OUTSIDE the catalog `yield` field (a verb payout, a loan fee, an event rebate) must still be measured against the owning item's `weeklyUpkeep` — a near-passive income stream that exceeds upkeep is an investment no matter which subsystem pays it. Concrete fixes for the owner to pick from: (a) set `museum_loan.cooldownWeeks` strictly above `MUSEUM_LOAN_WEEKS` (e.g. 20-week cooldown for a 12-week loan) so there's a real rest period and the *average* weekly fee falls below the $200 upkeep; or (b) drop `MUSEUM_LOAN_WEEKLY_FEE` below the diamond's $200/wk upkeep so a loaned diamond still costs money to hold, keeping it a prestige play rather than a rental business. Audit-tooling upgrade worth doing: extend `audit-economy.cjs` to sum EACH item's verb/loan income potential (loan fee × loan-weeks / cooldown-weeks, race/track EV net of entry) and assert the near-passive component stays below that item's `weeklyUpkeep`, so "verb income exceeds upkeep" becomes a named static finding instead of a qualitative-pass catch.
- Process finding (worth a fix): `npm run audit:weekly:full` is supposed to run the perf + money-conservation Jest suites, but when `node_modules` is absent it printed "Performance jest suite skipped (dependencies not installed)" / "node_modules is absent" and STILL exited 0 with "⚠ Weekly audit passed" — the dynamic layer silently no-ran while the summary read as a full pass. In an unattended routine that's a false all-clear. `run-weekly-audit.cjs --run-tests` should treat "asked to run tests but couldn't" as a hard non-zero failure (or at minimum a 🔴 in the report), not a green info line. Ran `npm ci` + the suites by hand this week to get the real dynamic signal.
- Lens reaffirmed (4th audit running): save/state and stability came back CLEAN; the one real finding was the economy/exploit lens again — and specifically the part of it that reads a NEW income surface against the feature's OWN stated balance law, not just the global ladders the static analyzer knows about.
