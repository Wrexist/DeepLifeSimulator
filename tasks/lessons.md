...
### 2026-06-23 - Weekly routine audit: clean verdict; project subagent defs missing from checkout

- What happened: ran the five-domain weekly audit (`npm run audit:weekly:full` + deep qualitative pass on the diff since the last audit `6bcf361..HEAD`). All automated invariants green, dynamic perf suite green (8.5s), money-conservation + save-migration + long-run save/load stress suites 44/44, `preflight:quick` clean. Both deep reviews (game-logic/economy and save/state) found **no blocking issues** — the week's diff (DeathPopup restructure, determinism via `seededRoll`, double-tap hardening, partial-migration tick guards) improves correctness without introducing time traps, mutations, schema drift, or economy printers.
- Harness gap: `CLAUDE.md` and the weekly-audit SKILL reference project subagents at `.claude/agents/game-state-reviewer.md` and `.claude/agents/save-system-auditor.md`, but `.claude/agents/` does not exist in this checkout. Had to run the deep pass with general-purpose agents using the documented framing instead. (The 2026-06-21 audit already "logged a harness gap" — same class of issue.)
- Non-blocking backlog (filed, not fixed this run):
  - 🟡 `JobActions.ts` street-job level-up *message* is predicted from the stale `gameState` snapshot while XP is applied atomically on `prev` in the updater — on a rapid same-week double-tap the displayed "level up!" text can disagree with the committed level. Cosmetic only (no economy/state impact). Fix: compute the message after the updater from `gameStateRef.current`.
  - ⚪ `applyPets.ts` sickness pre-roll buffer is 20; pet index 20+ shares a roll with `idx % 20`. Irrelevant at realistic pet counts and strictly better than the prior immunity bug.
  - 🟡 (pre-existing, unchanged since last audit) 20 `as GameState` casts in tests bypass `createTestGameState` (Hard Rule #3); many are in stress tests deliberately simulating partial/corrupt saves.
  - Design note (not a defect): with `weeklyEvents=false`, the tick clears `pendingChainedEvents`, dropping any in-flight story chain. Consistent with the flag's intent; confirm it's acceptable product behavior.
- Rule: when the audit playbook names a subagent file, verify it exists before relying on it; if `.claude/agents/` is absent, fall back to general-purpose agents with the SKILL's hunt-list as the prompt rather than skipping the deep pass.
