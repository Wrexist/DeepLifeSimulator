...
The DeepLife+ paywall rendered every figure â€” plan cards, CTA, legal disclosure,
"just $0.96/week", "SAVE 17%" â€” from `SUBSCRIPTION_CONFIGS`, a static map of
...
- **A config price is a build-time guess about a run-time fact.** The gem shop
  ...
  fallback needs to know it is one â€” `PlanPrice.fromStore` exists so the UI can
...
- **Derived claims inherit the trust level of their worst input.** The "SAVE
  ...
  has a direction too â€” a derived price rounds UP and a discount FLOORS, so
...
- **The dead-code tell: a reader with no writer.** `lib/offers/pricing.ts` and
  ...
  with a display string. Two correct, tested features â€” a discount badge and a
  currency-honest value line â€” were unreachable on every live storefront. The
...
- **"Not proven false" is not "true".** The trial claim was shown whenever
  ...
  ALL of Android, every build without RevenueCat keys and every failed lookup â€”
  ...
  in CLAUDE.md Â§7, where `undefined` (unknown) must stay distinguishable from a
  concrete default â€” `storeFreeTrialDays` returns `null` for unknown and `0` for
...
- **`setBusy(true)` is not a lock.** The purchase handler guarded on the `busy`
  ...
  would both pass. Same gate-then-act shape as Â§4.4's money bugs, just with a
...
`__tests__/render/SubscriptionModal.render.test.tsx` (asserts NEGATIVELY â€” no
...
- **A fix "across the board" is only as broad as the ratchet that guards it.**
  The 08-15 capture-across-updater sweep was real and thorough â€” inside
  ...
  (AdRewardOrb â€” a player watches a full rewarded ad, sees "Reward added!",
...
- **An allowlist coverage test certifies the past, not the present.**
  ...
  said nothing about the two added since â€” which ran bare, where a throw
...
- **In a shared working tree with parallel agents, stage explicit file lists,
  ...
  agent changing that file broke a suite its own validation never ran â€” a
...
- **A concurrency fix can reopen the hole it closes one layer up.** F-9 made
  `queueSave` await write completion by racing the drain promise â€” but the
...
- **A render test that accepts any tree is green on a crash screen.**
  ...
  its ErrorBoundary fallback â€” a perfectly valid tree, so "mounts without
...
- **A declared event with no emitter is the analytics form of dead code, and it
  ...
  says exist to measure "retention (D1/D7/D30) â€¦ and churn points". Three of
  them â€” `onboarding_step`, `tutorial_step`, `session_end` â€” were emitted by
  ...
  one** â€” the catalogue is not the wiring.
- **The cost was a measurement you cannot backfill.** "Play" cut a first-time
  ...
  retention figure below the 25th percentile of the peer set â€” and there was no
...
- **Check the DATE of the data against the date of the fix.** The retention
  numbers under investigation were for the week of Jul 13â€“19; Quick Start landed
  ...
  weeks earlier â€” the most expensive kind of work there is, because it looks
...
- **A narrow grep is a false negative generator, and I hit it twice in one
  ...
  analytics was unwired â€” the real API is `track(`, called from eight files.
  **Before reporting something as absent, search the whole repo for the symbol,
  not the directories you expect it in.** Â§8's rule about not trusting a finding
  without re-reading the source applies to your own findings first.
- **Emit outside the `setState` updater â€” the Â§4.4 rule is not only about
...
- **`inactive` is not the end of a session.** iOS raises it for a notification
  ...
  measured length of every session that survives one â€” understating the exact
...
- **A test that hard-pins `STATE_VERSION` fails on every correct future bump.**
  ...
  so â€” which is the tell that this is a pattern, not an accident. What a
  ...
  while its own name said "no longer short" â€” a floor written as an equality
  becomes a ceiling.
- **A bound that never binds proves nothing.** Three separate caps landed this
  ...
  child), and in each case the useful test is not "the result is â‰¤ cap" â€” that
  passes when the cap is unreachable â€” but "the uncapped value EXCEEDS the cap,
...
- **A per-source income cap can turn a "risky" feature into a safe one.**
  ...
  that already governs it â€” the answer changes the design, not just the risk.
...
- **A shipped, tested, context-exposed system with no call site is not shipped.**
  ...
  currency was unspendable in the app â€” a readout next to a locked door. This is
...
- **Two surfaces enforcing the same gate must read the same state.** `home.tsx`
  ...
  identical destination behind `tab:progression` â€” so the padlock read as
  ...
  money/weeks fallback â€” showing a lock the other screen doesn't. Rule: before
...
- **An id that differs between two screens is a dead link waiting to happen.**
  ...
  `featureUnlocks` registered both â€” the only layer that did NOT was the `?app=`
...
- **My own mistakes this round, both caught by tests I wrote first:** a
  ...
  keys â€” so it passed on noise, and two of its three assertions were vacuous.
  ...
  wrong, not the data. Rule: when a test parses source, assert the PARSE first â€”
...
- **A message bus with a gate on it is not a message bus.**
  ...
  `reward > 0` â€” correctly, so tips and warnings can't hijack the branded
  "ACHIEVEMENT UNLOCKED!" popup â€” and the reward was hard-coded 0 at all four
  ...
  Both halves were individually correct â€” the gate, and the helpers. Only the
...
- **Deleting a severity tier to fix a position bug throws away the one message
  ...
  The overlap was real; the remedy silenced every rejection on the Work screen â€”
  ...
  saying "a rejection used to be silentâ€¦ UX-2", so the bug it claims to fix was
...
- **`Math.max(x, 1)` inside a grant undoes the guard outside it.** The
  ...
  `computeWelcomeBackBonus` floors `daysAway` at 1 â€” so a second `onClose` in the
  same React batch saw `daysAway = 0`, clamped it back to 1, and paid another
  half-week of salary. The gateâ†’grant rule (Â§4.4) is not satisfied by stamping
...
- **My own mistake this session:** the first version of the stale-tab-copy guard
  ...
  Browser" and "the Miners tab" â€” legitimate sub-tabs *inside* an app. A
...
- **A zero-mean ARITHMETIC return is a negative GEOMETRIC one.** `simulateWeek`
  stepped stock prices with `price *= (1 + zÂ·Ïƒ)` and no drift term, which is
  `E[log ratio] â‰ˆ âˆ’ÏƒÂ²/2` per week. At the 8% weekly vol on TSLA/NVDA/META/NFLX
  that is âˆ’0.32%/week: driving the real pipeline for ten game years left 22 of 25
  symbols down with the median at 0.32Ã—, and at forty years four symbols sat on
  the $0.01 clamp. Because the walk is seeded on `weeksLived`, this was not
  variance â€” it was the same guaranteed collapse in every save on every device.
  Fixed with a log-normal step (`exp(Î¼ + Ïƒz)`) plus a volatility-scaled risk
  ...
  is finite. The old suite checked prices stayed positive and inside the clamp â€”
...
- **"Is it called?" is a different question from "does it work?", and only one
  ...
  calls the leaf helper directly â€” the exact failure the `applyBenefit`
...
- **A cap derived from the thing it caps is not a cap.** My first arrears
  ...
  it with `Math.max(carried, â€¦) * 3` â€” a ceiling that grew with the balance. A
...
- **Two constants describing one thing will disagree.** `WEEKS_PER_MONTH = 4`
  and `WEEKS_PER_YEAR = 52` (4 Ã— 12 = 48), and the tick used one for the week
  ...
  one line down by flooring the month-start week instead of ceiling it â€” caught
...
- **Three of my own errors this session, all caught by tests I wrote before
  ...
  compounded to ~8 000Ã— over a life; and an empirical "volatile names pay more"
...
- **Repo weight and download weight are different numbers.** I reported 67 MB of
  ...
  the tree. The actual problem was the 234 MB that DOES ship â€” over Google Play's
  200 MB base-AAB limit â€” and no preflight section looked at it. Rule: measure
...
- **`setGameState`'s updater does not run at the call.** I tried to thread the
  real confiscated amount out of a `setGameState(prev => â€¦)` into a message built
...
### 2026-07-20 - Weekly audit: GameState schema drift â€” 4 fields added to initialState AFTER the version bump, no migration/repair
...
- What went wrong: `luxuryItems` (Luxury & Collectibles, commit `5e3cdf1`) and `ambitionId` /
  ...
  so `migrations[22]` doesn't set them and `repairGameState` didn't backfill them â†’ every existing
  ...
  crash on the entire installed base â€” the exact "GameState drift" Hard Rule #3 exists to prevent.
- Why the static audit missed it: `audit-save.cjs` verifies migrations `[2..N]` are all *covered*
  and that `STATE_VERSION` matches the docs â€” both were true. It does NOT diff `initialState`'s field
...
- How it was found: the weekly-audit Crash/Save subagent traced each `initialState` field added this
  ...
  partial/CloudSync saves. `ambitionId` intentionally omitted â€” its default is `undefined`, so an
  absent key already equals the default. Updated DEV.md / WORKFLOW.md / CLAUDE.md to state v23.
- Rule: adding a field to `initialState.ts` is a THREE-part change that must land together â€” (a) a
  ...
  NOT safe drift â€” it's a latent crash waiting for the first non-guarded reader. Consider a static
  ...
  `.claude/prompts/*` the SKILL references still don't exist in-repo â€” ran the deep pass with
...
- Two code paths that reach the SAME outcome must produce IDENTICAL state, or one silently drifts.
  ...
  lets the tick resolve it â€” so a large fraction of marriages went through the auto path and got an
  ...
  diff them â€” a subsystem-equivalence snapshot test is the right home for this (it caught the diff
  cleanly on `-u`).
- One unguarded caller in an every-tick loop reintroduces a soft-lock class the resilience test
  ...
  with `prev.banking?.budgetSpend ? â€¦ : â€¦`, but the new weekly-tick `spendEvents` loop
  ...
  `prevState` â†’ "Next Week" silently no-ops (soft-lock). Fixed at the SOURCE (default
  `[...(banking.budgetSpend || [])]`) so all present/future callers are covered, and wrapped
  `runWeeklyBankingTick` in its own try/catch like the pulse/spark/stocks ticks â€” its crash surface
  ...
  subsystem tick whose failure aborts the whole week needs its own try/catch â€” check that every new
  tick step has one.
- `planWedding` charged its 25% deposit without re-checking affordability inside the updater
  (the same H-class atomicity gap the audit keeps closing) â€” a same-batch double-tap double-charged.
...
- Process note: the referenced project subagents/prompts (`.claude/agents/*`, `.claude/prompts/*`)
  do not exist in this repo â€” the SKILL points at them but they were never committed. Ran the deep
...
- Non-atomic gateâ†’grant money printers (same H-8/H-9 class the mega-audit keeps closing):
  ...
  `setGameState(prev => â€¦)` that returns `prev` when the gate no longer holds.
- TWO more fixed-size pre-roll buffers indexed by an uncapped collection (the petSickness class,
  ...
  index â†’ partner past index 20 immune to breakup; doctor-visit cure buffer (len 10) â†’ 11th+
  ...
  red flag, not a spec â€” re-verify it's intended, not just inherited.
- TWO unguarded `.length` reads on fields `repairGameState` does NOT backfill (`family.children`
  in ShareLifeCard's tagline, `curedDiseases` in CureSuccessModal) â€” crash-on-old-save. Note
...
- Process note: when two audit PRs overlap, the conflicted one is not worthless â€” diff it against
...
- What went wrong: `IAPService.applyBenefit` runs TWO grant paths in sequence for every
  ...
  `await`s `saveGame(true)` â€” persisting the credited state to the active slot â€” before
  ...
  credited the player 2Ã—. Flag products (perks, multipliers, ads-removed) are idempotent
  boolean sets, so they were unaffected â€” which masked the bug.
- Why it hid: all existing IAP tests (`iapMonetization.stress.test.ts`,
  `premiumPackIncome.test.ts`) exercised `applyProductToState` â€” ONE path â€” never the
  ...
  comment asserting the grant happens "exactly once per transaction" â€” the author removed
...
- How it was found: the weekly-audit economy subagent flagged it (MEDIUM); source-verified
  by tracing `applyBenefit` â†’ `stateUpdater` (IAPHandler) â†’ `applyBenefitToDisk`, confirming
  ...
  pack (2Ã—) before the fix. Fixed by capturing the in-memory updater's boolean result and
  passing `{ skipBenefitReapply: inMemoryApplied }` to `applyBenefitToDisk`, which then
  gates only the additive `applyProductBenefitsToState` re-apply (disk-only concerns â€”
  permanent perks, subscription fulfillment, transaction ledger, save â€” still run).
- Rule: when a benefit/grant has redundant apply paths (in-memory + disk, optimistic +
  ...
  Test the COMBINED entry point, not just the shared leaf helper â€” a redundant-path bug is
...
- What went wrong: the Legacy Pass module has two ways to bring a stale pass up to the live
  ...
  to a fresh empty pass when `pass.seasonId !== liveSeasonId` â€” it's a normalizer, not a
...
- Why it hid: every claim test operated WITHIN the current season (the happy path). The
  rollover/collection tests covered only the XP and reconcile paths â€” none drove a claim across
...
- Rule: when two helpers in the same module both "bring state to the current period" but one
  ...
  the in-period happy path â€” a rollover that's only exercised by one subsystem will rot in the
  others.

### 2026-06-24 - Cold-container false positive: perf jest "FAIL" was just missing node_modules (again)

- What went wrong: `npm run audit:weekly:full` reported a ðŸŸ  HIGH "Performance jest suite
  failed" that looked like a real blocking week-loop regression. The container had an EMPTY
  `node_modules` (fresh clone, no install), so `jest` died with "Preset ts-jest not found" â€”
...
- Why it hid: the static `npm run audit:weekly` (no jest) is green, so only the `:full` dynamic
  ...
  This is the inverse of the 2026-06-21 cold-container lesson (there jest was silently absent â†’
  false green; here jest can't load its preset â†’ false red).
...
- What went wrong: the weekly tick pre-rolls per-entity RNG into fixed-length arrays
  ...
  `undefined < 0.06` â†’ `false`: those pets become permanently immune to sickness (no crash, no
  error, just a balance/correctness drift that only shows up on a long, pet-heavy save).
- Why it hid: every test used â‰¤ a handful of pets (well under the buffer), and the refactor
  snapshot suite asserted byte-identical output for small inputs â€” none exercised an index past
...
- Rule: when a fixed-size pre-roll/lookup array is indexed by a collection whose size isn't
  ...
  relationships/diseases and >10 vehicles â€” apply the same wrap if those collections can grow.

### 2026-06-21 - "Missing" tooling already existed on main â€” fetch + check open PRs before building it

- What went wrong: the scheduled "weekly audit" routine prompt referenced `npm run audit:weekly`,
  `tasks/weekly-audit-<date>.md`, and `.agents/skills/weekly-audit/SKILL.md` â€” none of which were on
...
- Also true (still-valid sub-lessons): a piped `| tail` swallows the real exit code (`${PIPESTATUS[0]}`
  â‰  `$?`) â€” verify `node_modules` exists before trusting a green check on a cold container; and verify
  every subagent severity grade against source (this run's three real P2 fixes were each confirmed at the
  line â€” they survived the reconciliation because they were genuine code fixes, not tooling).
- Rule: before building tooling that looks "missing", `git fetch origin main` and scan open + recently
  ...
  to run the audit (reconstruct intent from equivalents) â€” but don't commit a parallel harness without
...
- What went wrong: three deep audit subagents (run as background agents, salvaged after a session suspend)
  ...
  over-graded â€” 0 genuine P0s. Examples: the "out-of-bounds NaN crash" (C3/C4) can't fire because
...
- How it was found: verifying each finding against the real code BEFORE fixing (the user chose "verify-first").
  Batch 1 (3 items) all fell to verification, then Batch 2 (2), Batch 3 (3), and C1 â€” 9 of 9.
- Rule: treat subagent/audit severity grades as UNVERIFIED LEADS, never ground truth. Source-verify each P0
  against the actual code path â€” call sites, sync vs async, batching, and existing mitigations â€” before
...
### 2026-06-15 - The $24.99 Premium Pack money multiplier was inert â€” dead flag written, real field not
...
- What went wrong: weekly income applies the money multiplier by reading `goldUpgrades.multiplier`
  ...
  `config.allUpgrades` / `config.everythingUnlocked` branches â€” which the Premium Pack
  (`moneyMultiplier: true`, no allUpgrades/everythingUnlocked) does not have. So the paid 1.5Ã— multiplier
  did nothing. A prior audit (round11 MON-3) even mis-concluded "the money mult IS delivered via the
  different goldUpgrades.multiplier" â€” it traced the write of the dead flag and the existence of a
  goldUpgrades write, but never the END-TO-END writeâ†’read chain for that specific product.
- Why it hid: a stress test (`iapMonetization`) asserted `settings.moneyMultiplier === true` after the
  purchase â€” i.e. it tested the WRITE of the dead flag, which "passed", giving false confidence. No test
  fed the purchased state through `computeWeeklyIncome` to confirm the income actually changed.
- How it was found: a "verify the mapping" task (roadmap H7) written as an END-TO-END regression test â€”
  apply the real product config, then run the real income calc and assert the 1.5Ã—. It failed (ratio 1.0),
  exposing the inert multiplier. Fixed by setting `goldUpgrades.multiplier` under `config.moneyMultiplier`
  in both paths.
- Rule: for monetization (and any writeâ†’read feature), test the END-TO-END effect, not just that a flag
  was written. A flag/field is only "wired" if the consumer reads THAT field. When the same effect is
  applied by multiple code paths (here: in-memory `applyProductToState` vs disk-apply â€” the "divergent
...
### 2026-06-15 - UI render tests did NOT need a jest-expo host â€” just gaps in the existing RN mock
...
- What was believed: `__tests__/integration/gameFlow.test.tsx` and `screenImports.test.ts` both stated render
  ...
  **0 `render()` tests across 254 components** â€” the biggest durability gap (per the 2026-06-15 roadmap).
- What was actually true: `react-test-renderer@19.1.0` is already installed, and `jest.setup.js` already mocks
  ...
  ts-jest/node env â€” screens just hit a few **mock gaps** that threw, not a fundamental host limitation.
- The specific gaps (all additive fixes to `jest.setup.js`): (1) `Animated.View`/`Text`/etc. were missing â†’
  `usePressableScale`'s `<Animated.View>` was `undefined` ("Element type is invalid"); (2) `Animated.sequence`/
  `parallel` returned objects without `.stop()` â†’ crash on unmount when a component stops an entrance anim;
  ...
  ts-jest (ts/tsx-only transform) can't parse â†’ mock them.
- Rule: to add render coverage in a ts-jest/string-mocked RN project, use `react-test-renderer` directly and
  fill mock gaps reactively (run â†’ read the throw â†’ mock â†’ repeat). Keep mocks ADDITIVE in `jest.setup.js`
  ...
  bad-import / Animated-misuse / provider-cycle crashes â€” but NOT navigator-level version-skew crashes (see
...
- Rule: after any edit to `.github/workflows/*.yml`, parse locally and confirm the workflow appears in GitHubâ€™s Actions tab without errors.

### 2026-04-20 - Full GameProvider tests in Node Jest

- What went wrong: `__tests__/integration/gameFlow.test.tsx` could not mount `GameProvider` under `@testing-library/react-native` with the repoâ€™s `jest.setup.js` RN string mocks; `useGame()` never ran and assertions saw `null` context.
...
- Pattern: Phase B sweep caught most weekâ†’weeksLived sites but missed read-side bugs where the write side had already been migrated. The asymmetry hides the bug because the field name `lastPostWeek` *looks* correct on both sides.
- Rule: whenever a field is named `*Week`, grep BOTH writes AND reads against the cyclic-vs-absolute axis. If the writer uses `weeksLived`, every reader must compare against `weeksLived` (not `state.week`). Treat `state.week` as a UI-display value only â€” never compare it against any stored field.
...
- What went wrong: `repairGameState(state)` mutated `state.stats`, `state.banking` etc. in place. Callers did `{...prev}` to give React a new top-level ref, but every nested ref was unchanged â€” selectors keyed on `gameState.banking` saw the same object identity and silently skipped renders, leaving the UI showing stale data after a "successful" repair. Looked exactly like a frozen UI.
...
- What went wrong: `DatingActions.ts fileDivorce` declared `immediatePaymentApplied`, `divorceDebtCreated`, `forcedStockLiquidationPaid`, `forcedPropertyLiquidationPaid` at outer scope, assigned them inside a `setGameState(prev => {â€¦})` updater, then read them after the call to format the log line and the user-facing divorce summary message. React batches/defers functional updaters, so the read sees the initial values (0) â€” the user sees "$0 immediate payment" in the divorce modal.
...
- Rule: never use a `setGameState(prev => {â€¦})` updater to assign closure-scoped variables that are read by code following the `setGameState` call. Compute audit/return values against the action's `gameState` snapshot first, then call `setGameState` with the precomputed objects. Reserve `prev` inside the updater only for spread-merging fields that other actions might touch concurrently (typically `prev.dailySummary`, `prev.family`, etc).

### 2026-05-30 - REVERTED: React.lazy() inside an expo-router screen crashes production iOS

- What went wrong: round 6 converted `app/(tabs)/computer.tsx` (17 sub-apps) and `app/(tabs)/mobile.tsx` (8 sub-apps) from eager `import X from 'â€¦'` to `const X = lazy(() => import('â€¦'))` with a `<Suspense>` fallback. Type-check and the local Jest suite passed clean. The EAS-built iOS production bundle then crashed at app launch with `Element type is invalid: expected a string â€¦ but got: undefined` inside the root navigator â€” the "Router Initialization Error" screen.
- Pattern: expo-router scans every `app/**/*.tsx` file at boot to register routes. That import walk wakes up the lazy wrappers' module identities even though the wrapped chunks haven't been rendered yet. In the minified Hermes production bundle, at least one of those dynamic `import('â€¦')` chains resolves through a path where the `.default` export is not unwrapped â€” or one of the transitive imports under a `lazy()` chunk is itself undefined â€” and React throws at the navigator render. Dev mode and JS tests don't reproduce this; only the production Hermes bundle does.
- Rule: do NOT use `React.lazy(() => import('â€¦'))` for components that an expo-router screen references at module top (the `apps[activeApp]` map pattern). If code-splitting is needed, defer the load via an explicit `require()` inside a `useEffect` AFTER mount, and gate the import behind an error boundary that surfaces failures with a useful message. Eager imports are the safe default for any component the router will see during boot.
- Guardrail in place: [__tests__/startup/screenImports.test.ts](../__tests__/startup/screenImports.test.ts) asserts every `(tabs)/*.tsx` file has a `export default` AND that `computer.tsx` + `mobile.tsx` contain no `React.lazy(() => import(â€¦))` patterns. CI will block any future regression.
...
- Rule: after any migration that introduces hook calls into an existing component, run `npx eslint <files> --quiet` before committing â€” never rely on type-check + tests alone. Place all new selector hooks in the component's existing hook block at the top, not at the site of the expression they replace.

### 2026-06-09 - `useGameState().setGameState` reintroduces the full-state subscription

- What went wrong: Batch 4 migrated `GemsStoreModal` to slice selectors but took `setGameState` from `useGameState()`. That hook subscribes to the whole `GameStateContext`, so the component still re-rendered on every state mutation â€” the migration looked complete but delivered zero isolation. Caught one batch later while planning TopStatsBar.
...
- Rule: migrated components must get write access from `useSetGameState()` (store-backed, stable, no subscription) and actions from the split action hooks (`useMoneyActions()`, `useGameActions()`, â€¦) â€” never from `useGame()`/`useGameState()`. Verification: after migrating, grep the file for `useGame(`/`useGameState(` â€” both must be absent.

### 2026-06-09 - Local type-check passed while CI failed: stale incremental tsbuildinfo

- What went wrong: PR #7 CI failed `tsc -p tsconfig.typecheck.json` with 5 TS18048 errors (`sm`/`pol`/`dw` possibly undefined in PulseApp.tsx and milestones.ts) that every local `npm run type-check` run during the session reported clean. Deleting `*.tsbuildinfo` and re-running locally reproduced all 5 â€” the incremental cache had skipped re-checking those files after the cast-removal edits changed inference in their dependencies.
...
- Rule: before pushing any commit that removes casts / changes type inference, run the type check cold: `rm -f *.tsbuildinfo && npx tsc --noEmit -p tsconfig.typecheck.json`. Also note: runtime guards like `safe(x?.field, 0) > 0` do NOT narrow `x` for TS â€” use `x?.field` again inside the branch instead of `x.field`.

### 2026-06-11 - `eas build --local` never auto-increments â€” duplicate CFBundleVersion rejected on submit

- What went wrong: `eas submit` for iOS failed with "You've already submitted this build of the app." `eas.json` had `cli.appVersionSource: "remote"` but **no** `autoIncrement` on any profile, and the failing pipeline (`.github/workflows/eas-build-local-ios.yml`) builds with `eas build --local`. Remote versioning made EAS ignore `app.config.js`'s existing `BUILD_NUMBER` hook, AND `--local` builds do not run the remote auto-increment (that only happens on EAS *cloud* builds) â€” so every local build baked the SAME `CFBundleVersion` and Apple rejected the duplicate.
- Pattern: `appVersionSource: "remote"` silently disables the local `ios.buildNumber` / `android.versionCode` values (remote becomes the source of truth), and the remote `autoIncrement` flag is a *cloud-build-only* feature. A `--local` + `remote` combination therefore has NO working increment path â€” it ships the same build number forever, and the failure only surfaces at submit time, not build time.
- Rule: any pipeline that uses `eas build --local` must manage the build number itself. Set `cli.appVersionSource: "local"` and compute a unique, monotonic `BUILD_NUMBER` (`scripts/next-build-number.mjs`: returns one higher than App Store Connect's latest build when `ASC_KEY_ID`/`ASC_ISSUER_ID`/`ASC_KEY_P8` are present, else `date +%s` epoch seconds â€” which also stays under Android's ~2.1e9 `versionCode` cap) so `app.config.js` bakes a fresh `CFBundleVersion` into each binary. Reserve `remote` + `autoIncrement` for the *cloud* `eas build` path only. After a failed submit you must REBUILD with a new number â€” the already-built `.ipa` can never be re-submitted as-is.
- **AMENDED 2026-08-14 â€” do NOT act on the `appVersionSource: "local"` half of this rule.** It was never applied (`eas.json` has always been `"remote"`), and applying it now would break the cloud path: `eas-build.yml` has no `BUILD_NUMBER` step and depends on remote + `autoIncrement`, so `"local"` would bake app.config.js's `"99"` fallback into every cloud build. The part of this lesson that fixed the bug and still holds is `scripts/next-build-number.mjs` + `app.config.js` minting the number locally; that works *with* `"remote"` â€” `eas-build-local-ios.yml`'s `submit-ios` job has been accepted by TestFlight repeatedly (verified 2026-08-12), which could not happen if `CFBundleVersion` were being ignored and repeated. See the 2026-08-14 entry at the end of this file.

### 2026-06-26 - Weekly audit reported a false ðŸŸ  high: perf jest suite "failed" in a fresh routine container

- What went wrong: the scheduled weekly-audit routine runs `npm run audit:weekly:full` in a freshly-cloned container. `node_modules` was not installed, so `npx jest __tests__/performance` died with "preset ts-jest not found" *before any test ran*. `audit-perf.cjs` caught that and reported it as a ðŸŸ  high "Performance jest suite failed", failing the whole audit (`âœ— ... 0 critical, 1 high`). There was no real perf regression â€” the suite passes in 4.5s once `npm ci` runs.
- Pattern: a dynamic check that shells out to a test runner conflates two distinct outcomes in one `catch` â€” (a) the suite ran and an assertion/timing budget failed (a real regression, ðŸŸ ), and (b) the runner couldn't start at all (missing deps, bad preset, no tests collected â€” an environment problem, not a regression). The routine harness (cron container) is exactly the env where (b) happens on every cold run, so the false blocker is recurring, not a one-off.
- Rule: before treating a shelled-out test failure as a finding, prove the harness actually ran. In `audit-perf.cjs`: gate the run on `depsInstalled()` (`node_modules/.bin/jest` + `ts-jest` both resolve) â†’ INFO-skip if absent; and in the catch, only emit ðŸŸ  when the output contains a real test summary (`/Tests:\s+\d+/`) â€” downgrade `preset .* not found` / `Cannot find module` / `No tests found` to INFO. The SKILL playbook's "false positive â†’ tighten the analyzer, don't suppress" applies: the fix is detection, not deleting the check. Operationally, the routine's SessionStart setup should `npm ci` so the dynamic backstop actually runs (it only adds value with deps present).

### 2026-06-29 - Weekly audit: two HIGH money printers static checks can't see (atomicity + unit mismatch)

- What went wrong: the deep qualitative economy pass found two repeatable money printers the static audit (`audit:economy`) passed clean over. (1) `ContactsActions.redeemFavor` gated on the stale `gameState`, credited cash in one `setGameState` call, then flipped the ledger in a SEPARATE call â€” two same-batch taps both passed the stale gate and both paid out (a positive credit never overdraft-rejects) while the ledger closed once. (2) `VehicleActions.cancelInsurance` charged a 6-month premium (`monthlyCost*6`) for a 26-week term but refunded `floor(monthlyCost*weeksRemaining/4)-25` â€” a 4-week "month" against a 26/6â‰ˆ4.33-week premium, so an immediate buy-then-cancel refunded up to 6.5 months of a 6-month policy: +$25..+$175/cycle, repeatable. Also `sellVehicle` lacked the inside-`prev` ownership re-check its sibling `sellItem` already had.
- Pattern: the static economy analyzer validates CONSTANTS (APR ordering, tax monotonicity, ladders, floors) but is blind to (a) grant/credit and state-flip split across two updaters â€” the H-8/H-9 same-batch double-tap race â€” and (b) refund/proration formulas whose weekâ†”month unit basis disagrees with the charge formula (the H-3 "refund returns more than was paid" class). Both compile, type-check, and pass the existing tests because those tests only assert flags/active-state, never the money delta.
- Rule: any action that grants value (cash, item, perk) AND mutates a ledger/ownership flag must do BOTH inside ONE `setGameState(prev => â€¦)` that re-checks the gating condition against `prev` (use `applyMoneyDelta` for the money leg so it shares the overdraft/ceiling guards), never gate on the outer stale `gameState`. For any refund/proration, prorate against the ACTUAL premium paid and ACTUAL term (`premiumPaid * remaining/term`) and clamp `refund â‰¤ premiumPaid` â€” never re-derive months with a different week-per-month divisor than the charge used. And every regression test for a money action must assert the exact `stats.money` delta across the buy+cancel / double-tap, not just the resulting flag.

### 2026-06-29 - IAP listener didn't share the foreground flow's in-memory dedup lock

- What went wrong: after the expo-iap migration, `runPurchaseFlow` (foreground) guarded against concurrent same-transaction processing with the in-memory `processingTransactions` Set, but `setupPurchaseListener` checked only the PERSISTED ledger (`isTransactionProcessed`). The persisted mark (`markTransactionProcessed`) is written at the very END of `applyBenefit`, after an async disk read/write â€” so if the listener fired for the same transactionId while the foreground grant was mid-flight, both passed the persisted check and both called `applyBenefit`, double-granting a consumable (gems/money).
...
- Rule: any value-granting action must fold the gate re-check + every state mutation (marker, cost, reward) into ONE `setGameState(prev => â€¦)` that re-reads the gate from `prev` and returns `prev` unchanged if it no longer holds; route the money leg through `applyMoneyDelta(prev, â€¦)` so it shares the overdraft/ceiling guards. Re-derive any deterministic roll from `prev` (the lineage-seeded RNG is stable within a batch, so the outer message and the inner authoritative recompute agree). Every regression test must assert the exact `stats.money` AND resource (energy) delta across a same-batch double-tap â€” thread one shared `setState` over the same stale snapshot passed to both calls â€” not just the resulting flag.

### 2026-07-01 - Double-grant bugs hide in DEAD components too (GemsStoreModal)

- What went wrong: PR #43 fixed the IAP consumable double-grant in `ShopModal` and `GemShopModal` (the service applies `config.gems` via the IAPHandler `stateUpdater`; the modals must NOT re-add locally). But `GemsStoreModal.tsx` still ran `stats.gems += totalGems` after `purchaseProduct` â€” a real double-grant of real-money gems. It survived because the component is imported NOWHERE (grep found only its own self-reference); the live gem modal is `GemShopModal`. So it was a dormant landmine, not a shipped exploit â€” but exactly the footgun to trip a future dev who wires it up.
...
- Rule: after fixing a class of bug (double-grant, unit mismatch, non-atomic gate), grep the WHOLE tree for the pattern â€” not just the wired call sites â€” including dead/duplicate components (`grep -rn 'stats.gems +' components/` etc.). Fix or delete the dead copy; leaving a live `+= <currency>` after `purchaseProduct` anywhere in the tree is a regression waiting to be re-mounted. Verify reachability with a component-name grep before down-grading a duplicate-grant finding to "not exploitable".

### 2026-07-03 - Weekly audit: RDActions.enterCompetition was the last gateâ†’grant that actually PRINTED money

- What went wrong: the deep economy pass found `enterCompetition` (`contexts/game/actions/RDActions.ts`) still non-atomic. The `alreadyEntered` + affordability gates were read from the stale outer `gameState`, the entry fee was charged via `deps.updateMoney` (dispatch #1), then the history entry was appended in a SEPARATE `setGameState` (dispatch #2) that never re-checked the gate. Two same-batch taps both passed the stale gate and both appended a duplicate `competitionHistory` entry (same `competitionId|entryWeek`). Unlike the self-charging double-tap actions (buyAccessory, purchasePassport, â€¦) this one PAYS OUT: `processCompetitionResults` loops every pending entry and does `totalPrize += prize` per entry, so the duplicate independently placed and its prize was summed â€” a repeatable, untaxed money printer (prizes are 10Ã—+ the entry fee). `applyHistory` marks BOTH duplicates completed, so nothing lingered to hint at the double-count.
- Pattern: same H-8/H-9 class the audit keeps closing (ContactsActions.redeemFavor, buyPet, PetActions/HobbyActions enterCompetition). The tell is purely structural: a gating read from the outer snapshot ABOVE the first `setGameState`, with the grant/marker/cost split across more than one updater. RDActions was overlooked because a PRIOR round (R10-1) had already made `processCompetitionResults` (the resolution/payout half) atomic â€” but the ENTRY half was never folded, and the two halves were audited separately. When a feature has a deferred payout (enter now, resolve N weeks later), BOTH halves must be atomic; hardening only the resolver leaves the printer open at entry.
- Rule: fold the gate re-check + entry fee (`applyMoneyDelta(prev, â€¦)`) + marker append into ONE `setGameState(prev => â€¦)` that re-reads the gate from `prev` and returns `prev` unchanged on the second tap. For any deferred-payout action, grep BOTH the enter* and the resolve/process* sides â€” a duplicate entry created at enter-time becomes a duplicate payout at resolve-time even when the resolver itself is atomic. Every regression test must assert exactly ONE entry appended AND the fee charged once across a same-batch double-tap.

### 2026-07-03 - The weekly-audit analyzer's own doc-drift check silently went dark after a file rename

- What went wrong: this week's tree renamed the dev docs (CLAUDE.md â†’ DEV.md, AGENTS.md â†’ WORKFLOW.md). `scripts/audit/audit-save.cjs` hard-coded `for (const doc of ['CLAUDE.md', 'AGENTS.md'])` for its STATE_VERSION cross-check. After the rename both files were absent, so the check hit the `src == null` branch and emitted `a.low('â€¦ not found', 'Skipping doc-version check')` for both â€” a WARNING that reads as noise, while the actual invariant (docs must state the canonical STATE_VERSION, drift has bitten this repo before) was no longer verified against ANY file. The audit's own safety check went dark and reported it as a low, not as "I can no longer see the thing I'm supposed to guard."
- Pattern: a static analyzer that references source/doc paths by hard-coded name silently stops enforcing its invariant when the target is renamed â€” the "skipping check" branch is indistinguishable from "nothing to check." This is the analyzer-integrity twin of the perf-suite lesson (2026-06-26): a check that can't run must not masquerade as a check that ran clean.
- Rule: when a file/symbol the audit scripts reference is renamed, grep `scripts/audit/` for the old name as part of the change. Point the check at the CURRENT names and distinguish "current doc missing" (worth an INFO/medium â€” the tree should ship it) from "legacy doc missing" (silent, expected). If NO doc satisfies the invariant, escalate to medium ("No dev doc states STATE_VERSION") rather than emitting per-file "not found" lows that bury the real signal.

### 2026-07-03 - IAPHandler resolved `true` on save-success regardless of whether the benefit applied

- What went wrong: the logic/stability pass found `components/IAPHandler.tsx` resolved the in-memory `stateUpdater` promise with an unconditional `resolve(true)` in the `saveGame().then(â€¦)` branch, ignoring the `applied` boolean from `applyProductToState`. `IAPService.applyBenefit` keys its `skipBenefitReapply` guard on that resolved value: when the in-memory apply FAILED (`applied === false`, e.g. an unknown/misconfigured product) but the save succeeded, the service saw `true`, skipped the additive disk re-apply, and the paid consumable (gems/money/youthPills) was silently never credited â€” a lost paid grant. (The `.catch` branch already resolved `false`, so only the apply-false + save-success path was wrong.)
...
### 2026-07-09 - Weekly audit: exposeCatfish was the last non-atomic gateâ†’grant, this time on reputation not money

- What went wrong: the economy pass found `SparkActions.exposeCatfish` still split across an updater + a trailing dispatch: the `setGameState(prev => â€¦)` appended a `catfishRecord` and bumped `totalCatfishExposed` with NO dedup on `profileId` (its sibling `reportProfile` guards `reportedIds.includes`), then `updateStats(setGameState, { reputation: 5 })` ran as a SEPARATE dispatch with no `applied` gate. Two same-batch taps â†’ duplicate record, double counter, +10 reputation for one catfish. Same H-8/H-9 structural tell the audit keeps closing, but on a bounded stat (reputation clamps 0-100) rather than money, so it graded MEDIUM not a printer.
- Pattern: the gateâ†’grant class is not money-specific. Any value-granting action whose grant is split across the updater and a trailing `updateStats`/`updateMoney` is double-tappable â€” reputation, followers, records, counters all leak the same way. The tell is identical; only the severity differs by what's granted.
- Rule: fold the dedup re-check into the updater against `prev`, capture an `applied` flag inside it (the PulseActions.composePost idiom), and skip the trailing `updateStats`/`updateMoney` when `!applied` (return a `success:false`). The regression test must fire the action TWICE against the same stale snapshot and assert exactly one record + the stat delta applied once. When sweeping for this class, don't stop at `updateMoney` â€” grep trailing `updateStats(`/counter bumps too.

### 2026-07-09 - Weekly audit: a concurrent audit PR had already fixed our top finding on main â€” rebase and re-scope before pushing

- What went wrong: this run's deep pass independently found the auto-wedding path (`applyScheduledWedding`) never set `marriageWeek`/`anniversaryWeek`/`livingTogether` (a HIGH) and fixed it inline. But between branching and pushing, merged PR #49 fixed the exact same bug on `main` â€” and better, extracting a shared `buildSpouseRecord` factory used by both the manual and auto paths (the very "factor the shared shape into one helper" refactor this audit's own lesson recommends). Our branch was cut from a now-stale base, so the PR went `mergeable_state: dirty` (conflict in `applyScheduledWedding.ts` + the equivalence snapshot). The wedding fix was fully redundant; only the `exposeCatfish` fix + tests were still novel.
...
- Rule: before opening an audit PR, `git fetch origin main` and diff your finding's file against the CURRENT tip â€” not the base you branched from. If `main` already fixed it, rebase onto `main`, drop the redundant change, and re-scope the PR to only what's still novel (keep your tests if they add coverage the merged fix lacks â€” e.g. a direct semantic assertion vs an opaque snapshot). Resolve `dirty` by resetting onto the fresh `main` and re-applying just the novel diff; force-with-lease is fine when the discarded commits were never merged.

### 2026-07-10 - Weekly audit: the new server IAP endpoint accepted SANDBOX receipts in production (free premium)

- What went wrong: this week's big new surface was `server/iap-verify/api/verify.js` (the receipt-verification backend the app fails-closed against). The deep IAP pass found `verifyApple` looped `for (const env of [Environment.PRODUCTION, Environment.SANDBOX])` and returned `true` if EITHER verified. A StoreKit *sandbox* transaction is free to obtain on a production build (sign a Sandbox Apple ID into device Settings) and its signed JWS verifies cleanly under `Environment.SANDBOX` â€” so a normal user could unlock `deeplife_lifetime_premium` (and every gem/perk product) for $0. The server literally decoded the environment and granted anyway. Separately, the `ALLOW_SOFT_LAUNCH` stopgap granted ANY well-formed >20-char string for a known productId, one stray env var away from a total bypass on the live endpoint, "protected" only by a bearer token that ships in the client JS bundle.
- Pattern: the money exploit this week was NOT in the game's action layer (which the audit has hardened round after round against the gateâ†’grant double-tap class) â€” it was in freshly-added *backend/server* code that the five static analyzers don't scan at all. A new trust boundary (a verification server) is exactly where the next revenue leak hides once the client-side exploits are closed. Also: "verifies under some environment" â‰  "is a real production purchase" â€” the environment IS part of the authorization decision, not an implementation detail to loop over.
- Rule: when a release adds a server/verification endpoint, audit it as its own domain â€” treat every `grant`/`verified:true` path as a money printer until proven otherwise. Accept PRODUCTION receipts only in prod; gate SANDBOX behind an explicit `IAP_ALLOW_SANDBOX` flag that staging/TestFlight/review builds set and production never does. Make every soft-launch/bypass stopgap fail-closed in production (require a second deliberate `*_IN_PROD` flag). Never rely on a bearer token sourced from an `EXPO_PUBLIC_*` var as a real gate â€” it's in the bundle. Grant *amounts* must come from a server-side product table, never the client-supplied receipt/quantity (this part was already correct â€” keep it that way).

### 2026-07-10 - Weekly audit: the gateâ†’grant double-tap class also lives in UI components, not just action modules

- What went wrong: the logic pass found `components/LifeMomentModal.tsx` `handleChoice` read `gameState.lifeMoments?.pendingMoment` from the stale render closure and applied `updateMoney`/`updateStats`/karma via separate dispatches BEFORE the `setGameState` that clears `pendingMoment`. Two same-frame taps both passed the `if (!pending) return` gate â†’ double grant (e.g. "Bank the windfall" = +$5,000 â†’ +$10,000; karma choices double their delta). This is the identical structural tell the audit keeps closing in `contexts/game/actions/*`, but it had gone unscanned because it lives in a `components/*.tsx` modal, and the 4â†’20 life-moments expansion this week added a no-cost cash-windfall choice that turned a latent double-apply into a real (if low-frequency) printer.
- Pattern: the audit's grep for the gateâ†’grant tell has historically targeted `contexts/game/actions/`. But any component whose `onPress` reads a pending/gate value from the render closure and then fires `updateMoney`/`updateStats` across multiple dispatches is the same bug. The action-module hardening pushed the remaining instances of the class into the UI layer, where money/stat helpers (`updateMoney`) can't be trivially folded into one `setGameState(prev=>)`.
- Rule: extend the gateâ†’grant sweep to `components/**/*.tsx` â€” grep for `onPress` handlers that read a `pending*`/gate field from `gameState`/props and call `updateMoney(`/`updateStats(` before clearing the gate. When the grant helpers are external dispatches that can't be folded into the authoritative clear, latch on the item's unique id with a `useRef` (set synchronously at the top of the handler, checked before applying) so a same-frame double-tap resolves exactly once â€” the "disable after first tap" fix, done in a way that's robust to React's render timing.

### 2026-07-10 - Weekly audit: a weekly-tick subsystem grew crash surface but stayed outside the try/catch its siblings have

- What went wrong: `runPoliticsWeeklyTick` was called at `GameActionsContext.tsx:~1470` with NO try/catch, while the banking and stocks ticks that bracket it in the same `nextWeek` updater ARE each wrapped with a fallback. This week added +67 lines of election-resolution logic to the politics tick, growing its throw surface â€” and a throw there would abort the entire `nextWeek` updater and soft-lock "Next Week". The static perf audit even reported it obliquely ("48/49 subsystems inside try/catch") but the one unwrapped subsystem wasn't called out by name.
- Pattern: the weekly-tick resilience invariant ("one subsystem's failure must not abort the whole tick") is enforced unevenly â€” new subsystems get added without the try/catch wrapper their siblings have, and the "N/M wrapped" static metric hides *which* one is bare. A subsystem that gains new code this week is exactly the one whose missing guard now matters.
...
- What went wrong: winning political office (`PoliticalActions.ts`) sets BOTH `currentJob:'political'` and pushes `careers[political]` with `accepted:true`. That made the generic weekly career-salary path (`applyCareerSalaryAndPenalty.ts`) pay `POLITICAL_CAREER.levels[level].salary` as a WEEKLY amount, while `passiveIncome.ts` ALSO paid the same salary as ANNUAL Ã· WEEKS_PER_YEAR. Both fed `totalIncome` every week. POLITICAL_CAREER salaries are authored as annual figures (President = 100000), so the generic path paid a President ~$100k/WEEK (~$5.2M/yr) on top of the intended ~$1,923/wk â€” and because office-loss only zeroes `politics.careerLevel` (not `currentJob`/`careers[].accepted`), the ~$100k/wk never stopped after being voted out. A textbook money printer gated behind "win an election."
- Pattern (two compounding): (1) the same salary table is consumed by two income paths with DISAGREEING units (one treats it weekly, one annual) â€” whenever a value feeds more than one accrual path, the paths must agree on units AND on ownership, or they double-count. (2) The economy/money-conservation stress suite is blind to income that only unlocks off the default loop: winning the presidency isn't in the default simulation, so 33/33 money-conservation tests stayed green while a President printed millions. The static economy analyzer (savings<loan APR, tax monotonicity, miner ladder) also can't see it â€” it audits constants, not the tick's income aggregation. It took the *game-logic* lens (trace what fires on `currentJob:'political'`), not the *economy* lens, to catch it.
- Rule: any income source that is ALSO delivered by `passiveIncome` (political, rent, mining, stocksâ€¦) must be excluded from the generic per-`currentJob` career-salary path â€” one owner per income stream. When you add a salary/price table consumed by multiple accrual paths, assert the unit (weekly vs annual) at every consumer and pick a single owner. For the audit itself: money-conservation stress tests must exercise the OFF-default income unlocks (hold each political office for a week and assert the weekly credit equals the single intended figure), because a printer behind a feature-unlock is invisible to a default-loop conservation test. When the economy lens comes back clean, still run the game-logic lens over "what pays out when `currentJob`/a career flag is set" â€” the two lenses catch different halves of the economy.

### 2026-07-13 - Weekly audit: crypto & dark-web ticks were the last two subsystems running outside try/catch

- What went wrong: continuing the 2026-07-10 "bare weekly-tick subsystem" thread, the crypto tick (`GameActionsContext.tsx` ~1342) and dark-web tick (~1484) were the only subsystem ticks in the `nextWeek` updater still NOT wrapped in try/catch (banking/stocks/politics/pulse/spark/hustle all are). Both self-guard *missing* top-level slices (`?? initial`), and dark-web even carries a comment claiming it avoids throws â€” but that self-guard only normalized `activeJobs`/`recentEvents`, leaving `vendors`/`skills`/`laundering`/`listings` exposed, and crypto left `coinMarkets.btc` (halving path) and `market.openOrders` iteration unguarded. A present-but-null sub-field (CloudSync merge / hand-edit / corruption â€” the exact class `repairGameState` names as a threat but only handles for whole-slice-missing) throws, and a throw there aborts the whole updater â†’ "Next Week" soft-locks. The static perf audit reported it only as an opaque "50/51 subsystems inside try/catch" â€” the ONE bare subsystem wasn't named, and here there were effectively two.
- Pattern: a subsystem that "self-guards" is not equivalent to one wrapped in try/catch â€” the self-guard defends the fields the author remembered, and silently rots as the schema grows new sub-arrays. The belt-and-suspenders wrapper (try/catch + carry-over fallback) is what actually makes the resilience invariant hold regardless of which sub-field is null. The "N/M wrapped" static metric hid *which* ones were bare.
- Rule: every subsystem tick call in `nextWeek` gets its own try/catch with a carry-over fallback (`nextX = prevState.x`), full stop â€” don't rely on the subsystem's internal null-guards to substitute for it. When a normalize/self-guard block lists specific sub-fields, it must cover EVERY iterated/indexed slice of that type (grep the tick's operations for `for (const â€¦ of dw.X)` / `dw.X[`), not just the two the author hit first. Upgrade the static audit to NAME the unwrapped subsystem(s) rather than emit a bare "50/51".

### 2026-07-16 - Weekly audit: the "bare weekly-tick subsystem" class recurred a 4th time â€” and this time I finally made the static audit NAME the gap

- What went wrong: this week added FOUR new subsystem calls into the `nextWeek` updater â€” `applySubscriptionsForWeek` (in-game sub billing, 1365), `applyContentMemberships` (streaming memberships, 1552), `applySavingsGoals` (1574), and `expireFavors` (favor-ledger expiry, 1596) â€” and NONE were wrapped in the per-subsystem try/catch that every sibling (banking/stocks/politics/crypto/darkweb/pulse/spark/hustle) has. The concrete HIGH: `expireFavors` (`lib/contacts/favors.ts`) did `ledger.favors.map(...)` while the call site only guarded `prevState.favorLedger` truthiness â€” a present-but-partial `favorLedger: {}` (CloudSync merge / hand-edit / interrupted migration) throws `undefined.map`, and unwrapped in the updater â†’ outer catch at ~2033 returns `prevState` â†’ `weeksLived` never advances â†’ PERMANENT "Next Week" soft-lock. The other three were defensive enough that I couldn't construct a throw today, but they broke the categorical invariant. The static perf audit again reported only a bare "52/53 subsystems inside try/catch" â€” the offenders weren't named.
- Second finding (MEDIUM, different class, same root save-shape): dc3e337's crash-fix sweep for legacy `sparkApp` lacking `premium` missed two sibling call sites â€” `SparkActions.rewindLastSwipe` (`sp.premium.perks.rewindLastSwipe`, line 273) and `likeBackFromLikedYou` (`sp.premium.perks.seeWhoLikedYou`, line 468). Both read the RAW `gameState.sparkApp` (no `ensureSpark` backfill) and only guarded `if (!sp)`, so an old save with a `premium`-less sparkApp crashes with "Cannot read properties of undefined (reading 'perks')" on tapping Rewind / a Liked-You entry. A partial-save fix that touches ONE consumer of a slice must grep for EVERY consumer of the same sub-field â€” the class re-lives at the sites the sweep skipped.
- Economy verdict: CLEAN. Streaming (finalize/start/tick), in-game subscriptions, and the ad-reward orb were all independently traced by subagents + by hand â€” the gateâ†’grant double-tap class is architecturally closed (every grant folds into the authoritative `setGameState(prev=>)` updater via `applyMoneyDelta`, with the gate re-checked against `prev` inside it). No printer. Non-blocking LOWs filed: immediate stream/video payouts bypass the $75k/wk soft-cap that only the passive aggregator enforces (earned + hard-bounded by the 5-actions/week cap, so a balance decision not an exploit); subscription weekly billing has no `BANKRUPTCY_FLOOR` guard (anti-player, drains to $0, unlike loan autopay); subscribe actions lack an already-active re-entry guard (anti-player double-charge, mitigated by modal gating); dead `deps.updateMoney` param in ContentActions.
- Rule: the recurring lesson finally got its tooling fix â€” `audit-perf.cjs` P2 now NAMES the unwrapped subsystem calls (`â€” unwrapped: <names>`) instead of a bare "N/M", so the next bare subsystem is actionable at a glance rather than buried in a count. The block-count delta (42â†’46 here) is the honest signal that new wraps landed. When the count is "M-1/M", read the named residual: a pure self-guarding helper like `applyMoneyDelta` (returns null, never throws) is an acceptable residual; a real `apply*/run*/process*` subsystem tick is not. Keep folding: every NEW subsystem call added to `nextWeek` gets its own try/catch + carry-over fallback in the same commit that adds it â€” the invariant is categorical, not a function of whether you can construct a throw this week.

### 2026-07-17 - Weekly audit: the "bare weekly-tick subsystem" class recurred a 5th time (disease/pet/vehicle/luxury) â€” and I learned the static P2 metric can't see it

- What went wrong: the chronic-disease management loop (11e87e6) added new logic to `applyDiseasesForWeek` but left its call site in the `nextWeek` updater OUTSIDE any per-subsystem try/catch â€” as were its three neighbors in the same block (`tickPetsForWeek`, `applyVehiclesForWeek`, `applyLuxuryItemsForWeek`). All four iterate player-growable arrays (`diseases`/`pets`/`vehicles`/`luxuryItems`) and carry real partial-save throw surface â€” the concrete one: `applyDiseases.ts:107` `[...(input.prevDiseases || [])]` throws "not iterable" on a truthy non-array `diseases` (CloudSync merge / hand-edit / interrupted migration), BEFORE the helper's own `Array.isArray` guard. Unwrapped, that throw falls to the outer updater catch which returns `prevState` â†’ `weeksLived` never advances â†’ "Next Week" fails that week (mitigated: the outer catch surfaces an error toast rather than white-screening, and `repairGameState` normalizes `diseases` to `[]` on next load â€” so it's failed-week-until-reload, not a permanent brick). Fixed all four with their own try/catch + carry-over fallback, preserving the deliberate money-mutation order so the success path stayed byte-identical (308 subsystem snapshots unchanged).
- Second thing I learned (audit-tooling reality check): I tried to make `audit-perf.cjs` P2 "name the gap" by excluding the whole-updater try (whose catch returns prevState = the soft-lock) from the guarded-ranges set. It backfired: doing so drops guarded coverage from 52/53 to 23/53 and names ~30 subsystems (computeWeeklyIncome, applyRentAndHousing, applyCrimeTick, applyWeeklyEvents, â€¦) that ALSO rely solely on the outer wrapper. The truth the experiment exposed: the code does NOT actually wrap every subsystem individually â€” only the higher-risk, partial-save-prone ones get inner try/catch, while ~30 pure-ish calculators lean on the outer net. P2 is a *tolerant smell check* (guardRatio â‰¥ 0.6, outer-wrapper counts) by design; a strict "every call in its own try" rule would flag a 30-item backlog, not a regression. Reverted the P2 change.
- Rule: keep wrapping incrementally â€” every NEW or NEWLY-EDITED subsystem call in `nextWeek` that iterates a player-growable array gets its own try/catch + carry-over fallback in the same commit that touches it (the actionable, non-flooding version of the categorical invariant). The static P2 check genuinely CANNOT distinguish "safe outer-wrapper reliance" from "dangerous outer-wrapper reliance" â€” it counts the outer try as a guard â€” so it will keep reporting a reassuring 52/53 while a freshly-edited array-iterating tick sits bare. The real detector is the human/subagent game-logic lens tracing "which newly-touched tick iterates a growable slice without a leaf try" â€” run it every week on the diff, because the number will lie.
...
- Review refinement (CodeRabbit, valid): my first cut grouped pets + vehicles + luxury + pet-food into ONE shared try/catch. That's a half-measure â€” those subsystems mutate `weeklyCtx.newStats.money`/happiness IN SEQUENCE, so if a LATER one (luxury) throws after an EARLIER one (vehicle maintenance / pet-death penalty) already mutated `newStats`, a shared catch reverts the later subsystem's OUTPUT (`updatedVehicles`â†’prevState) while the cash/happiness mutation stays applied â†’ state-vs-cash desync, and the player gets re-charged/re-penalized for the same week on the next tick. Rule: when wrapping N sequential subsystems that each mutate shared `newStats`, give each its OWN try/catch so a failure isolates to that subsystem and never rolls back a sibling whose side effect already landed. Also: the carry-over fallback must self-heal â€” `?? []`/`|| []` preserves a truthy non-array (the exact throw case) and re-throws every week until reload; use `Array.isArray(x) ? x : []` so the bad shape is replaced THIS tick.

### 2026-07-22 - Weekly audit: migration/repair asymmetry â€” `realEstateActivity` had a v22 migration but no `repairGameState` mirror (the static save audit can't see per-field repair parity)

- What went wrong: `realEstateActivity: []` (`initialState.ts:75`, a top-level concrete-default array) was backfilled on the version ladder by migration 22 (`saveMigrations.ts:619`) but was NEVER mirrored into `repairGameState` (`utils/saveValidation.ts`) â€” zero matches in the file. This is exactly the CLAUDE.md save-format rule (b) asymmetry ("set a value in the migration AND mirror it in repairGameState") that Hard Rule #3 exists to catch. It is not an active crash â€” every consumer already guards with `?? []` (`GameActionsContext.tsx:808`, `RealEstateApp.tsx:338`) â€” but a partial save already stamped at v23 (CloudSync merge / hand-edit) that is missing the key is healed by neither path: the wholesale migration skips it (version already current) and repair has no branch for it. Four v22 Wave-A NESTED fields have the same gap at LOW severity (`travel.passportMilestones`, `socialMedia.followerHistory`/`scandalRiskScore`, `gamingStreaming.perkTier`/`lastMemberWeek`/`hypeStreak`) â€” all guarded reads, filed not fixed. Fixed the top-level `realEstateActivity` with a 6-line mirror of the adjacent `luxuryItems` block.
- Pattern: the static save analyzer verifies the version-consistency invariants (STATE_VERSION across code+docs, contiguous migration coverage [2..N], repair/factory PRESENCE) but it does NOT cross-check each concrete-default field in `initialState` against BOTH migration and repair coverage â€” so a field can be migration-covered, factory-covered (createTestGameState spreads `...initialGameState`, so (c) is structurally auto-satisfied for every field), and still silently miss the repair mirror (b). The audit's own v23 fields (luxuryItems/ambition*) got both treatments and passed; the OLDER v22 additions were the ones that rotted. Guarded `?? []` reads at every consumer are what keep this at MEDIUM instead of a crash â€” which is also why a purely runtime test never surfaces it.
- Rule: when a field with a concrete stored default is added to `initialState`, its migration backfill AND its `repairGameState` mirror must land in the same commit (rule (b)), for NESTED fields too â€” the parent-subsystem repair block must list every new concrete-default sub-array/scalar, not just the ones the author hit first (same "self-guard rots as the schema grows" failure as the bare-tick class). Audit-tooling upgrade worth doing: extend `audit-save.cjs` to enumerate every `key: <concrete-default>` in `initialState` and assert each appears in BOTH `saveMigrations.ts` and `saveValidation.ts` (skipping `undefined`-default keys like `ambitionId`), so the migration/repair asymmetry becomes a named static finding instead of a subagent catch.
- Economy note (NOT fixed â€” design call for the owner): channel-membership income (`applyContentMemberships.ts`, new v2.5.7) is credited post-tax at `GameActionsContext.tsx:1801`, capped $75k/wk and idempotent (`lastMemberWeek`), but never enters `totalIncome` so it escapes progressive tax â€” WHILE the same creator app's ad-revenue stream flows through `passiveIncome`â†’`totalIncome` and IS taxed. It is consistent with the established post-tax-credit pattern for crypto (`:1698`), hustle (`:1657`), and stocks (`:1929`), so it is not a new regression or a printer; taxing it would require risky tick-reordering (tax computed at `:836`, membership at `:1795`). Left as a filed balance decision rather than changing money-flow in an unattended routine. Lens rule reaffirmed: economy came back CLEAN on printers/double-grants/free-debt-erasure; the only real fix this week came from the SAVE lens, not the economy lens.

### 2026-07-24 - Weekly audit: the monetization wave's real gap was the ECONOMY lens, not save/state â€” a daily premium-currency claim gated only on a device-clock day-string (farmable)

- Static + save-drift both came back CLEAN (the save subagent confirmed the entire DeepLife+ / gem-shop / Skill-Mastery wave added ZERO fields to `initialState.ts`, persisting via the sanctioned optional-undefined `settings` pattern like `ambitionId`, so no migration/version bump was owed; the only ðŸŸ¡ was the pre-existing cosmetic `as GameState`-in-tests count). The one real finding (HIGH) came from the economy/exploit lens: `claimDailyGems`/`canClaimDailyGems` (`contexts/game/actions/SubscriptionActions.ts`) gated the daily gem drop SOLELY on `settings.deepLifePlusLastGemClaim !== todayKey`, where `todayKey = utcDayKey(new Date())` â€” a pure UTC day-STRING derived from the device clock, with no monotonic timestamp. Gems are the PAID premium currency (sold via `deeplife_gems_*` IAPs), so this minted paid currency for free: set the device clock forward a day â†’ new day key â†’ claim 250 (member)/20 (free) again, repeat arbitrarily; rolling BACKWARD also worked since the guard was a pure inequality, not `todayKey > lastClaim`. Compounded the perfect-week bonus (fabricate a full Monâ†’Sun window of keys â†’ extra full daily drop). This violates the playbook's own H-3 note: "Daily/real-time claims should guard against `Date.now()` manipulation."
- Fix (two layered guards â€” the FIRST cut was incomplete, see below): (1) STRICT DAY-KEY MONOTONICITY â€” `todayKey` must be strictly LATER than the stored `deepLifePlusLastGemClaim` (keys are `YYYY-MM-DD`, so lexicographic `<=` is chronological); and (2) a MONOTONIC epoch high-water mark `settings.deepLifePlusLastGemClaimAt` (optional, undefined-default â†’ no version bump, matches the wave's own pattern) that also rejects a `nowMs` below the stored mark (minus a 5-min NTP skew tolerance), stamped `max(previous, now)` so it never decreases. Both guards live in one pure primitive predicate `canClaimDailyGemsFor(lastKey, lastAt, todayKey, nowMs?)` that BOTH the reducer and the `DailyGemClaim.tsx` CTA call, so the button and the reducer can never disagree about the tolerance. `nowMs` is optional so legacy 2-arg callers still work (epoch guard skipped); the real call site passes `Date.now()`, and the CTA shows the settled chip (not a dead button) exactly when the shared predicate says ineligible.
- What the FIRST cut got wrong (CodeRabbit CRITICAL, valid â€” caught in review): the epoch high-water mark ALONE, with a 5-min skew tolerance and only a same-day-STRING guard, still allowed an unbounded ALTERNATING-ADJACENT-DAY farm across midnight: claim 23:59 (mark=23:59) â†’ claim 00:02 next day (day key differs, 3 min > mark so allowed, mark=00:02) â†’ rewind to 23:59 (day key differs from 00:02's key, and 23:59 is only 3 min below the 00:02 mark â€” INSIDE the 5-min tolerance, so the epoch guard passes) â†’ reclaim yesterday's key â†’ repeat the two timestamps forever. The tolerance that protects a benign NTP nudge is exactly the window an attacker oscillates inside. The strict day-key monotonicity (guard 1) is what actually closes it â€” once you've claimed key `D`, no key `<= D` is ever claimable again â€” and it makes the epoch mark defense-in-depth rather than the primary bound.
- Pattern: a client-only game (no server clock â€” which is why save integrity leans on HMAC tamper-detection) cannot FULLY stop an offline clock-FORWARD cheat, but the two guards together close every BACKWARD vector (same-day, whole-day-rewind, and the sub-tolerance alternating-midnight farm) and make forward-farming self-limiting (advancing the clock advances both marks, so returning to real time locks the cheater out until real time passes their furthest claimed day). Deeper lesson: when you add a tolerance to a monotonic guard to avoid a false-positive, prove the tolerance window itself isn't farmable by oscillation â€” a scalar high-water mark + tolerance is oscillation-farmable across a category boundary (here midnight) unless a SEPARATE strictly-monotone key (the day string) also advances. The static economy audit checks constant LADDERS (APR ordering, tax monotonicity, miner prices) but has no notion of a real-time/daily claim surface â€” this class only surfaces under the exploit-lens qualitative pass (and the exploitable gap in the FIRST fix only surfaced under adversarial review â€” worth having a skeptic re-derive the exploit against the patch, not just the original code).
- Rule: any real-time/daily/cooldown reward (especially one paying premium currency) must gate on a STRICTLY-MONOTONE claim key (reject `todayKey <= lastKey`), not merely `!==`, AND persist an absolute epoch high-water mark; never gate solely on a clock-derived day/hour STRING, and never let a skew tolerance be the only thing standing between two claims. Share ONE predicate between the reducer and every UI that gates on it. Audit-tooling upgrade worth doing: extend `audit-economy.cjs` to flag any reducer that reads `utcDayKey`/`new Date()`/`Date.now()` for claim ELIGIBILITY without a strict `<=`/`>` day-key comparison against a persisted last-claim key, so "clock-string-only daily claim" becomes a named static finding instead of a subagent (or reviewer) catch. Lens reaffirmed (3rd audit running): the save lens caught the last two weeks' real bugs, this week the economy/exploit lens did â€” run all five every time; the clean lenses are not the ones that will bite.
...
- Static + save-drift + logic/perf all came back CLEAN on their own lenses (the only ðŸŸ¡ was the pre-existing cosmetic `as GameState`-in-tests count; the save subagent confirmed every recently-added field â€” `luxuryItems`/`luxuryHoldings`/`hasPilotLicense`/ambition* â€” is covered across all four legs and the v24/v25 migrations are gap-free). Both real findings came from the economy/exploit lens, and both live in `lib/luxury/verbs.ts` â€” the Phase-4b "verbs" wave that added things you DO with a trophy. The catalog's core invariant ("every `yield.weekly` is set BELOW its item's `weeklyUpkeep`, so a full collection still net-costs") is genuinely upheld by all 12 catalog rows AND by `yieldAppreciation.test.ts` â€” but that test only sums `getTotalLuxuryYield`, which reads the static `yield` field. Verb income is a SEPARATE channel (`getLoanIncome` added on top of yield in `applyLuxuryItems.ts:74`, and `resolveRace` purse credited via `LuxuryActions`), so it bypasses the only guardrail.
  - HIGH â€” museum loan: the `museum_diamond`'s "Loan to a museum" verb cost $0 / 0 energy with `cooldownWeeks: 12` == `MUSEUM_LOAN_WEEKS: 12`, so it re-armed the exact week the loan lapsed â†’ continuous coverage. $4,000/wk fee vs the diamond's $200/wk upkeep = uncapped, untaxed +$3,800/wk (~33%/yr) on a $600k asset for tapping one button every 12 weeks. Fix: fee â†’ $120/wk, below the item's upkeep, so continuous re-arming still net-costs. The draw is the prestige (+4 rep/loan), not the cash.
  - MEDIUM â€” racehorse: "Enter a race" credits the purse EXCLUDING the $25k entry, but the PLACE purse was $30k > entry, so a win AND a place both profit (2 of 3 outcomes) â†’ +$5k/race base EV, climbing with form. The code comment literally claimed "the entry is lost on every run that is not a win or a place" while the numbers said otherwise. Fix: place â†’ $15k (softens, doesn't cover the entry), win â†’ $70k, so base EV is negative and only a win profits; a maxed champion only edges positive per race, which upkeep-net-of-yield eats over the cooldown.
- What the tests got wrong: both verb tests asserted only single-payout MAGNITUDE ("best case < $200k", "fee == the constant") â€” never per-cycle EV or income-vs-upkeep. One test (`verbs.test.ts` "the prize covers the entry") actively ENSHRINED the racehorse exploit by asserting place >= entry. A test that pins the buggy number is worse than no test. Corrected it and added the two missing assertions: base-form race EV <= 0 over the {win,place,unplaced} distribution, and museum fee strictly below the diamond's upkeep.
- False-positive worth recording: the logic subagent flagged (SUSPECTED, medium-ish) that `calculateNetWorth` in `preTick.ts` omits luxury, concluding "the appreciation/market-value machinery has zero effect on the net worth that gates prestige." It does not â€” there are FIVE net-worth implementations, and the canonical one is `netWorth()` in `lib/progress/achievements.ts`, which DOES include luxury (`getTotalLuxuryMarketValue`, resale-haircut + appreciation + condition). That `netWorth` is what prestige, prestige points, leaderboard, ambitions, bail cost, ad rewards, and stats all read (via `statisticsTracker.calculateNetWorth` â†’ `netWorth`). `preTick.calculateNetWorth` is a legacy, more limited helper used for secondary decay inputs; `ShareLifeCard` and `retirementCalculator` carry their own luxury-omitting copies. So `createLuxuryProperty`'s `price:0` ("already counted through luxury resale") holds â€” no double-count, no missing-count. Lesson: when a subagent says "X isn't counted," grep for EVERY implementation of the thing before believing it â€” a duplicated calculation means "the one I read omits it" is not "the system omits it." Filed the 5-way net-worth duplication as a LOW consistency smell (a $120M collection shows in prestige/bail/ads but not the share card or retirement projection).
- Rule: any luxury/asset SIDE-CHANNEL income (verb payout, loan fee, event purse) must obey the same "below the item's upkeep / non-positive base EV" bar the passive `yield` field does, and the balance test must assert it THROUGH the channel the money actually flows (per-cycle EV, income-vs-upkeep), not just single-payout magnitude â€” because `getTotalLuxuryYield` only sees the static field. Audit-tooling upgrade worth doing: extend `audit-economy.cjs` to enumerate luxury verbs/loan constants and flag any whose best-case or expected per-cooldown-cycle net income exceeds the owning item's `weeklyUpkeep`, so "verb out-earns its trophy" becomes a named static finding. Lens reaffirmed (4th audit running): the economy/exploit lens caught this week's real bugs; the save lens caught the prior two weeks'. Run all five every time â€” and re-derive the exploit against the code, because a comment claiming "this can never be a printer" is exactly where the printer hides.

### 2026-07-28 (b) - Backlog sweep: the "filed not fixed" pile is where the audit's own tooling debt hides

- What I did: swept every finding the recent audits recorded as REAL but left unfixed, rather than running another discovery pass. The pile was six items across three audits (2026-07-16 economy LOWs, 2026-07-22 save parity, 2026-07-28 net-worth duplication) plus the three "audit-tooling upgrade worth doing" notes those same audits ended with. Every one was still present in the source â€” a finding filed with a file:line and no owner does not decay into a false positive, it just waits.
- What the tooling upgrade paid for immediately: the 2026-07-22 lesson asked for a static check cross-referencing each migration-backfilled concrete default against `repairGameState`. Built as `audit-save.cjs` V8, it reproduced the four NESTED v22 fields the human pass had already named â€” and then found FIVE OLDER ones nobody had looked for (`wantedLevel`, `processedIAPTransactions`, `pursuits`, `weeklyPursuitPractice`, `legacyPass.ownedCosmetics`). `wantedLevel` had real teeth: `JobActions.ts:316` does `prev.wantedLevel + (job.wantedIncrease || 1)` with no `?? 0`, so a partial save missing the key turns it into NaN and every success chance reading it (`Math.min(25, wantedLevel * 3)`) goes NaN too. The check's precision came from ONE design decision: `initialState.ts` is the authority on what "the default" is, so a migration that DERIVES a value (`lastEventWeeksLived = state.weeksLived || 0`) or converts legacy data (`challengeStreak`) is not flagged â€” filtering on that dropped the raw 9 hits to 5 genuine ones with no allowlist.
- Rule for new static checks: prove the check FAILS before shipping it green. All three new analyzers (save V8, economy E10 clock-string claims, economy E11 luxury-verb income) were run against the restored historical buggy values â€” $4,000 museum fee, $30k place purse, `todayKey !== lastKey` â€” and each tripped with the right message; then the probe was reverted. A check authored against already-fixed code and never seen red is an assertion that the current source equals itself.
- Economy note: the two subscription LOWs were both ANTI-player, which is why they survived four audits â€” the exploit lens hunts printers, and a bug that quietly overcharges the player trips nothing. Weekly billing had no `BANKRUPTCY_FLOOR` guard (drained to $0, unlike `applyLoanAutopay`), and `subscribeVerifiedPro`/`subscribeSparkPremium` re-charged full price for the plan you already held, resetting an annual `paidThroughWeek` to now+52 instead of extending it. Add "does this overcharge or under-deliver to the player" as an explicit pass of the economy lens; a money bug does not have to favour the player to be a money bug.
- Duplication note: the 5-way `netWorth` split resolved into three real answers, not one. `ShareLifeCard` was simply WRONG (cash + bank + real estate at PURCHASE price, contradicting every other surface) â†’ now reads the canonical `netWorth()`. `fireTracker`/`retirementCalculator` held byte-identical copies with a DIFFERENT and defensible basis (income-producing / liquidatable assets only â€” a 4%-rule projection should not tell you a yacht funds your retirement) â†’ extracted to one `lib/statistics/planningNetWorth.ts` with the basis written down, numerically unchanged. `preTick.calculateNetWorth` is snapshot-locked and stays. Rule: dedupe by first asking whether the copies MEAN the same thing; collapsing a deliberate second basis into the canonical one is a balance change wearing a refactor's clothes.

### 2026-07-29 - Full six-domain audit, executed: what the fixes taught that the findings didn't

- Context: a 12-agent audit (six domain passes, each re-checked by a separate adversarial verifier) produced 29 confirmed findings; all 29 are now closed across six phases. The findings documents are `tasks/full-audit-2026-07-28-findings.md` and `-plan.md`. What follows is only what EXECUTING them taught â€” the findings themselves are already written down.
- **The biggest bugs were all in code no test could reach.** GL-1 (event chains latched forever after one chain, locking out every future chain for the life) and GL-5 (`family.spouse` survived a breakup) both lived inline inside the `nextWeek`/`resolveEvent` React callbacks. `eventChains.test.ts` existed, but it hand-BUILT an `activeEventChain` and only exercised `getNextChainEvent` â€” nothing ever played a chain from stage 0 to its end, so the completion branch was unreachable in play AND unreachable in test. Extracting the decisions into pure `advanceEventChain` / `resolveFamilySpouse` helpers is what made the bugs visible; both now have tests that fail against the old logic. Rule: when a finding's root cause is "the branch was unreachable", the fix is not just the branch â€” it is moving the decision somewhere a test can drive it. A bug that hid in a callback will hide there again.
- **A fixed `mockReturnValue` cannot catch a double-roll.** The layoffs event drew `Math.random() < surviveChance` twice â€” once for `effects`, once for `special` â€” so ~48% of resolutions contradicted themselves (keep the job AND eat the laid-off penalty, or collect the survivor's reputation AND be fired). My first test mocked a constant and PASSED against the buggy code, because both draws read the same value and agreed by accident. Only a mocked SEQUENCE (`mockReturnValueOnce(0).mockReturnValueOnce(0.99)`) exposes it. Rule: to test that N draws became one, you must vary the draws; asserting on a single frozen value tests nothing. The strongest form is `expect(Math.random).toHaveBeenCalledTimes(1)`.
- **"Prove the check red" keeps finding more than the check was written for.** Every new analyzer this cycle was run against the pre-fix tree before shipping green â€” and two of them immediately found work nobody had scoped: `audit-save` V8 (migrationâ†”repair parity) turned 4 known gaps into 9, including `wantedLevel`, which `JobActions` increments with no `?? 0` so a partial save turns it into NaN; `audit-perf` P5 (zero-importer reachability) turned 13 named dead modules into 26, including a SECOND `applyLegacyBonuses` and a duplicate `FamilyBusinessSystem` sitting beside the live ones. Deleting also cascades â€” `audioLibrary` was the only importer of `audioManager`, which the check caught on the next run. Rule: a static check pays for itself on the first run or it was written too narrowly; and after a dead-code sweep, re-run the reachability check rather than assuming one pass converged.
- **Two fixes were nerfs that needed their mitigation in the same release.** econ-1 (luxury sold at catalog sticker while net worth counted condition-adjusted value, so selling a damaged trophy RAISED net worth and prestige points) shipped in one commit with reach-2 (insurance and restoration had zero call sites â€” the risk system was one-way loss with no counterplay). Shipping the nerf alone would have made every incident unanswerable. Rule: when a finding's own risk note says another finding is its counterplay, they are one release, not two.
- **Reporting fixes must not become balance changes.** recap-1's obvious fix â€” fold luxury yield into `totalIncome` so the recap adds up â€” would have routed it through `calculateIncomeTax` and retroactively taxed it. The yield went into the DISPLAY fields only. Same shape as the CLAUDE.md note about the store-version/binary-version split: the honest-looking unification is the one-way door. Rule: before making a number consistent, check which consumers of that number are money paths.
- **Dead code is where the wrong answer hides â€” and the audit proved it twice.** PERF-4 and PERF-5 were both a LIVE system whose only other implementation was unreachable. PERF-5's was worse than dead: `applyRelationshipGain` (charisma / socialMaster) had no production consumer at all, so a purchased skill node promised faster bonds and did nothing. Deleting the orphan made that visible; wiring it into `updateRelationship` (gains only â€” skills never soften a betrayal) is what made the purchase real. Rule: an orphaned module is not just clutter, it is a place where a feature can appear implemented.
- **Anti-player money bugs need their own lens.** econ-4 (an uncapped CASH ad faucet) was found by the exploit lens, but its siblings â€” subscription billing draining past `BANKRUPTCY_FLOOR`, and re-subscribing re-charging full price while resetting an annual term â€” survived four audits because the exploit lens hunts printers and these overcharge the PLAYER. Rule: run the economy lens twice, once asking "can the player mint money" and once asking "can the game overcharge or under-deliver".

### 2026-07-29 (b) - Save-system audit anchored on a real player report: what a lost save actually teaches

- Context: a player mailed in a prestiged save replaced by a Week 1 / Age 18 / Generation 1 character that validated clean â€” 0 errors, 0 warnings, no error or warn logs. An 11-agent audit (five domain passes, each finding adversarially verified, synthesised by a separate orchestrator) returned 46 raw â†’ 42 confirmed, 11 refuted. All 41 still-present findings are now closed across seven phases; `tasks/save-system-audit-2026-07-29-findings.md` and `-plan.md` hold the details. What follows is only what the incident and the fixes taught.
- **"Valid: yes, Errors: 0" was the most informative line in the report.** A save that validates perfectly and is still wrong is not corrupt â€” it is the wrong save. That single observation ruled out the entire corruption/migration surface before any code was read, and pointed straight at the write path. The fingerprint then closed it: money 200, age 18, weeksLived 0, careers 30, items 8, relationships 2 are `initialGameState` field-for-field, and NO scenario starts at age 18 with cash 200, so it could not have been built by onboarding. It was the untouched boot state, autosaved from the main menu (fixed in 2.5.7, after the player's 2.5.3 build). Rule: when a report says the data is valid, stop looking for corruption and start asking which writer produced it â€” and diff the payload against `initialState` before anything else.
- **I found a complete, plausible root cause that was NOT the incident â€” and it was still a real bug.** Independently of the agents I traced a full chain: the death screen navigated into onboarding setting no slot, `OnboardingContext` defaulted `slot: 1`, `flowGuard` never checked a slot, and `Perks` closed the gap with `state.slot || 1`. Every step verified in source. The agents confirmed it as SAVE-OW-2 â€” and separately proved it was not what happened to THIS player. Rule: a chain that reproduces the symptom is not thereby the cause; keep hunting until the payload's fingerprint is explained, and ship the other bug anyway.
- **The audit's most valuable finding was against code I had written an hour earlier.** SEC-1 flagged `slotSafety.inspectSlotForNewLife` â€” my own uncommitted last-line-of-defence â€” for the exact flaw it was written to prevent: `readSaveSlot` returns the SAME `null` for "nothing stored", "failed CRC32/HMAC" and "the read threw", so my `free` branch would have called an unverifiable save empty and deleted it. One HMAC key rotation (a live plan) would have made every slot on every device look free to overwrite. The module's own header even claimed "any failure to read resolves to `unreadable`, never `free`". Rule: run the audit against the fix, not just the original code â€” and treat a doc comment asserting a property as a claim to verify, especially when you wrote it.
- **The same conflation lived in five places, and fixing the callers is not fixing the bug.** `findFirstEmptySlot`, `checkIfAllSlotsFull`, `slotSafety`, `validateSaveSlot` and `purgeSlotIfPhantom` all read that null as "empty". Two other modules (`probeSaveSlotBlob`, `phantomSaveCleanup`) had the rule RIGHT and even documented it verbatim â€” "callers must never treat 'unknown' as safely overwritable" â€” but the fix never propagated. Phase 1 patched the callers; Phase 4 went to `doubleBufferLoad` and made the outcomes distinguishable (`none` / `unverified` / `unknown` + a `blobPresent` that a thrown read reports as `true`). Rule: when the same misreading appears in 3+ callers, the API is the bug. And a correct rule written in one module's comment does not propagate itself â€” encode it in the type.
- **Every recovery mechanism in the app was inert, in four independent ways, and each one hid the next.** Backups were written on every save but `restoreFromBackup` had zero callers (BRC-1); it wrote to the legacy single key while the loader reads the double buffer, so a restore was a silent no-op that returned `{success: true}` (BRC-1b); the 5-deep ring plus an unthrottled 2-minute autosave meant the whole recovery window was about ten minutes of play (BRC-2); and the "pre-save backup" snapshotted the state being WRITTEN, not the one being replaced (BRC-3). The one UI state literally labelled "Recovery Needed" offered exactly one action: Delete. Rule: a backup system is only as real as its most recently exercised restore. If nothing calls the restore path, assume it does not work â€” and test the round trip through the production LOADER, not through the key the restore happened to write.
- **The anti-cheat gate was aimed at the wrong person.** `canRestoreBackup` was written for an in-run rewind and applied to every restore, so `continueAsChild` bumping `generationNumber` made every backup from the run that just ended permanently unrestorable, and because the autosave keeps running while the death screen is up, the ring filled with dead-state backups that became the only LEGAL restores. Its catch failed closed "for security" â€” trading a single-player progression exploit against permanent data loss for someone whose save is already broken. Rule: separate INTENT (recovery vs rewind) before applying progression guards, and default an integrity check to fail-open when the failure mode on one side is unrecoverable data loss and on the other is a single-player exploit.
- **The test I wrote as an oracle found more than the audit did.** SEC-8 reported that the hand-rolled SHA-256 wrote a wrong 64-bit length word (`>>> 56` is `>>> 24` in JS). I fixed that and wrote a test comparing against `node:crypto` â€” which still failed, revealing a second defect nobody had found: the ipad/opad blocks were built as a JS STRING and handed to a `sha256` that UTF-8-encoded its input, so every pad byte â‰¥ 0x80 silently expanded to two bytes. Self-consistent, so it signed and verified fine forever, but not HMAC-SHA256 for ANY input. Rule: for anything with a published specification, assert against a reference implementation rather than against your own understanding of the defect â€” a self-consistent wrong algorithm passes every round-trip test you will think to write.
- **Correcting a signature is a fleet-wide data event.** Fixing the digest alone would have orphaned every save on every device. It shipped with the key-list work (SEC-2): the original construction survives VERBATIM as a verify-only fallback, verification accepts every configured key, and each save re-signs on its next write. `EXPO_PUBLIC_SAVE_HMAC_KEY` now takes a comma-separated list, newest first. Rule: any change to how a save is signed needs a verifier for the OLD form in the same release, and a rotation story before the key is ever rotated â€” a single bundled key with a `!==` compare is not a security posture, it is a single point of total loss (and it signs paid entitlements too, so it takes real purchases with it).
- **Two guards vanished under a mock.** `isWritableSlot` originally lived in `saveQueue`, so every suite that mocked the save queue silently lost it â€” one such suite went red the moment the guard started being enforced. Moved to `utils/slotNumber.ts`, a leaf with no imports. Rule: a validity guard consumed across module boundaries belongs in a dependency-free leaf; if mocking a neighbouring module can delete your guard, it is not a guard.
- **A refusal must be distinguishable from an absence.** Three separate defects were the same shape: a save from a newer build returned bare `null` â€” the value an empty slot returns â€” so the menu said "No save data foundâ€¦ start a new game" over an intact save (MR-4); `validateSaveSlot` hardcoded `exists: false` on the null path, making its own corruption messaging unreachable (PIPE-8); and four write-path guards turned "I don't know which slot this is" into "write it to slot 1" (SAVE-OW-6). Rule: never encode "refused", "failed" and "absent" as one value, and never let an unknown destination resolve to a default one â€” the correct failure mode for an unknown target is not to write.
...
- **Comparing the same step across commits separated infra from code in one call.** `list_workflow_jobs` returns per-step `started_at`/`completed_at`, so the Test step's duration is directly comparable run to run: `25368d6` 83s âœ…, `57dff30` 106s âœ…, `b2ab87a` 109s âœ…, `c402247` 60min+ hung. A 40x overrun on a docs-only diff is not a regression, it is a stuck runner. The local full suite then confirmed it â€” 349/350 suites, 4,347 tests, 135s, exit 0 on exactly `c402247`'s tree. Rule: before debugging a hung job, diff the *step timings* against the last green run of the same workflow. If the slow step is one your diff cannot reach, stop reading the diff.
- **The suspicious change was the innocent one, and CI had already proved it.** The only code/test edit since the last green run was swapping a runtime `require('@/lib/config/featureFlags')` for a hoist-safe `mockIsFeatureEnabled` (a review suggestion). That looked like the obvious culprit for a Jest hang â€” `jest.mock` is hoisted above the `const`, so it *smells* like a TDZ trap. But it shipped in `57dff30`, whose Test step ran 106s and passed. Rule: when a hang appears N commits after a suspicious change, check whether the suspicious commit's own run went green before theorising about it. The build history already ran the experiment.
...
weekly â€” the tick's clamp band is documented in `$/member/week`, `initialState`
...
ever had â€” a balance change wearing a bug fix's clothes. Removed instead; the
same condition already had an honest warning a few lines down. Compare GL-3,
where the promised effect WAS implemented â€” the difference is that GL-3 was a
...
- Two suites asserted a removed string was absent, and matched the fix's own
  explanatory comment â€” which necessarily quotes the string to explain the
...
## 2026-08-01 â€” I shipped a fix pattern, then measured it and found it unsound

Working the C-9 ratchet down, I converted `PetActions` (8 functions) and then
`VehicleActions` (9) from the C-8 shape â€” reject inside the updater, then
`return { success: true }` unconditionally â€” to a "pessimistic capture":

    let applied = false;
    setGameState(prev => { â€¦; applied = true; return next; });
    if (!applied) return { success: false, â€¦ };
...
state was correct; only the report was wrong â€” the inverse of the bug I was
fixing.

**The measurement** (`__tests__/refactor/updaterTimingContract.test.tsx`):
React runs the FIRST functional update of a batch **eagerly**, at call time â€”
...
**CLAUDE.md Â§4.1 already said this.** "Values computed inside a `setGameState`
updater are not visible outside it â€” don't assign to an outer variable from
...
**What I kept and why.** `PetActions` and C-8 stay: for a single tap â€” the
overwhelmingly common case â€” the eager path makes them correct, and they were
...
report â€” the C-10 `SkillTreeModal` shape) rather than at more capture.

**The estimate was wrong twice, in both directions.** The audit said ~15
functions; my first detector said 86; the corrected detector says 62 â€” the
...
## 2026-08-01 â€” 62 instances of a bad shape turned out to be 2 bugs
...
guard, reachable only by a second tap in one batch â€” where reporting failure is
...
Solar â†’ "Upgraded to Solar Panels", no charge, no change) and `buildRDLab` â€”
...
defects in that population was 2. The sweep was worth running â€” it found the
two â€” but presenting its raw output as severity would have justified a 62-site
refactor of critical action code for almost no player benefit, which is exactly
the kind of churn the priority order (Correctness â†’ Simplicity â†’ Root causes)
is meant to prevent. Read the candidates before quoting the number.

---

## 2026-08-05 â€” a 44pt target in the wrong place is still unhittable
...
had already fixed â€” it carried `minTouchTargetStyle`, `hitSlopToMinTarget` and
...
it is drawn before concluding the fix already landed â€” and when a surface is
...
## 2026-08-05 â€” render smoke tests that pass on the crash screen
...
renders instead. That screen is a perfectly valid tree â€” and the assertion in
...
allowlist, so any unlisted icon was `undefined` â†’ "Element type is invalid").
...
the first time someone uses an entry that is not on it â€” prefer one that cannot
...
## 2026-08-05 â€” a stored field nothing writes is not state

The Family header read `gameState.lifeStage` and rendered "Teen Â· Age 21" on a
21-year-old. `lifeStage` is assigned exactly once â€” `initialState.ts` seeds it
from `getLifeStage(18)` â€” and no birthday handler, weekly subsystem or scenario
...
its WRITERS before its readers** â€” the same question ("who writes this?") is what
exposed `child.familyHappiness`, the headline right next to it, as another field
with no writer at all.

## 2026-08-05 â€” 73% of a TestFlight release was a macOS runner waiting
...
Run #50 of `eas-build-local-ios.yml`, step by step:
    ...
    EAS Build iOS (local)   17m 26s   â† the actual compile
    Upload .ipa artifact         4s
    Submit to TestFlight    53m 15s   â† eas submit, waiting on EAS's queue
    ---------------------------------
    total                 1h 13m 12s

The submit step does about two seconds of work â€” it uploads the .ipa and
schedules a job â€” and then blocks until EAS finishes talking to Apple. All 53
...
**AMENDED 2026-08-19 â€” read that as an objection to losing the SIGNAL, not to
...
*alone* â€” the sentence above is exactly right about that, and the two steps are
...
belongs on the cheapest runner that can hold the connection â€” and any job that
waits on an external service needs a `timeout-minutes`, or the failure mode is
six hours of billing.

---

## 2026-08-06 â€” "carried across prestige" is a list somebody has to remember

Building prestige tiers 2â€“5 meant persisting four small things (Vault,
Endowment, Trials, Seat) and making them survive a life boundary. That is not a
save-format problem â€” the save carries them fine. It is a `prestigeExecution`
...
both paths call â€” here `applyDynastyTransition` (`lib/dynasty/transition.ts`) â€”
and the test asserts the hook appears on **both** paths, by count. The death â†’
...
= oldPoints + reward` is idempotent â€” re-running the transition on the same save
...
## 2026-08-06 â€” A test suite that HANGS tells you almost nothing, so instrument the process, not the source
...
timeout did not fire" is not a missing signal â€” it is the signal. It rules out
...
and named the wrong cause, because the change in it was a pure deletion â€”
...
1. **Run it under `node --inspect` and interrupt it.** Node 22 has a global
   ...
   Launch `node --inspect node_modules/jest/bin/jest.js â€¦` directly â€” putting
...
2. **CPU-profile it** (`Profiler.start` / `stop` over the same socket). The
   ...
   `throwException`, with **no application frames at all** â€” which says React is
...
3. **Count what React is doing.** Patching `scheduleUpdateOnFiber` to throw
   ...
   ~1.4M-call cycle through the same fibers. That combination â€” millions of
   `beginWork`, almost no `scheduleUpdateOnFiber` â€” is React restarting a render
...
`Promise.resolve().then(() => require(â€¦))`, so it can only settle on a
microtask â€” and the render harness renders inside a **synchronous** `act()`,
...
- A lazy component must be MOUNTED behind a condition, never mounted always with
  `visible={false}`. That defeats the point of `lazy()` â€” it defers the paint by
  a tick and still pays for the graph on every mount of the host screen â€” and
...
- **Count before declaring something a blocker.** In the same session, "settle
  ...
  wrong in the same direction â€” estimated scope, never measured.
- Restore any `node_modules` you patch, and `diff` to prove it.

---

## 2026-08-06 â€” an anti-exploit invariant left a second door open
...
(d5daaf8) hardened the tier-SWAP path â€” `resolveRentHome` carries `missedWeeks`
...
move out (free) â†’ re-rent, and the clock is back to zero while `overdueBalance`
...
Fix: gate re-entry in `canRent` â€” a landlord won't sign a new lease while the
...
a record is only as durable as the record â€” any code that rebuilds or discards
...
## 2026-08-07 â€” A defensive accessor that ENUMERATES fields is a trap

`lib/mail/state.ts`'s `getMailState` is the safe read layer every consumer goes
through â€” it repairs a missing or malformed slice into a valid empty one rather
than letting the week loop throw. It was written as an enumeration:

```ts
return { messages: â€¦, lastGeneratedWeek: â€¦, address: â€¦ };
```
...
through this function, and came out `undefined` â€” so paying to rotate
...
```ts
return { ...mail, messages: normalise(mail.messages), â€¦ };
```
...
## The same day â€” `patchMessage` compares by reference, so a no-op patch is not a no-op
...
check â€” and a spread always produces a new reference. So re-reporting a message
...
"the values are the same" â€” the helper cannot see that.

Same family as the gateâ†’grant bugs in Â§4.4: a guard that looks like it holds,
against a comparison that cannot see what you assumed it could.

## The same day â€” a test written from the same wrong model as the fix guards nothing
...
not the PROPERTY that prevents it â€” and the property is the whole point, because
...
## The same day â€” an audit finds the bugs the tests were never asked about
...
and assumed to be once.** And two are this repo's oldest pattern â€” built,
...
happens the SECOND time?" â€” second tap, second week, second application, second
read of the same accessor. That question found five of these six.

## 2026-08-09 â€” `flex: 1` inside a `maxHeight`-only sheet is zero, and a broken modal is a soft lock

Player report: "game soft locked on the life skills tab", with a screenshot of
the Activity Commitments modal drawn as a ~60px sliver under the status bar â€”
...
`header + 0 + footer` â€” and there is then no free space left for `flexGrow` to
...
with it â€” there is no tapping past it, and no back gesture on iOS. Every
...
**The rule:** `maxHeight` alone does not make a parent bounded â€” pair it with
...
## 2026-08-09 (same day) â€” the same defect, eighteen more times, in three spellings
...
It found **eighteen** more sites. Not one of them was a new mistake â€” they were
...
| `<ScrollView contentContainerStyle={â€¦}>` | no `style` prop at all to match against |
...
`View` capped at `maxHeight: scale(200)` holding description â†’ effect â†’
requirements â†’ **Unlock button**. Measured at base scale that column is ~218px,
~235px with a two-line description. A `View` does not scroll and the modal shell
is `overflow: 'hidden'`, so the button was clipped â€” the primary action of the
...
<View style={{ maxHeight: '90%' }}>        {/* bound */}
  ...
  <ConfirmButton />                         {/* fixed â€” NEVER inside the scroller */}
```
...
that class, the instance is the smaller problem â€” re-derive the sweep from the
...
is safe only because its copy is fixed, short, and authored â€” not because the
shape is sound. If those strings ever become dynamic or localized, it breaks
exactly like the others.

## 2026-08-07 â€” a new weekly subsystem landed outside the guard, again
...
pass found the one real thing. `applyArrears` â€” the settlement reducer added
with the rental/arrears feature â€” was called bare at `GameActionsContext.tsx`,
the ONE weekly `apply*` not inside `guardTick`. It routes every input through
`safe()` and only touches `Math.min/max/round`, so it cannot throw *today* â€” but
...
This is the same class Â§4.3 and this file already record five times over (the
...
source â€” but only for the historical thirteen it enumerates, so a fourteenth
subsystem added later slips straight past it.

Fix: wrap `applyArrears` in `guardTick('arrears', â€¦)` with an honest fallback
...
the ones that existed the day the test was authored â€” otherwise the guard rots
one new subsystem at a time, which is precisely how the original thirteen
accumulated.

---

## 2026-08-09 (same day) â€” a growth plan written without reading the code asked us to break the App Store rules
...
this-week must-do â€” Â§4.1, the rating prompt â€” was the one item that would have
...
**Why it is wrong.** That is *review gating* â€” pre-screening players and
...
deliberately refused. *(The guideline number in that comment reads 1.1.7 â€”
...
**Why it is wrong twice over.** The plan's premise â€” that the prompt fires at
naive moments and needs moving to peak-positive ones â€” was already false. The
prompt is a three-file system that does more than the plan asks for:

- `utils/reviewMoments.ts` â€” scores every candidate beat 0..1 and drops
...
- `utils/ratingPrompt.ts` â€” wall-clock cooldown as the primary guard,
  specifically because a game-week cooldown would burn all three of iOS's
  yearly asks in one long session.
- `components/ReviewPromptHandler.tsx` â€” a headless store subscriber rather
  than a call inside a reducer, because React can invoke an updater twice and
  one beat would fire two asks.

Adopting Â§4.1 would have replaced that with a worse system and a rejection
risk.
...
1. **A plan that describes our own product is a claim, not a brief.** Check
   ...
   button (`components/DeathPopup.tsx:569` â†’ `lib/legacy/obituaryGenerator.ts:123`),
...
2. **Domain-plausible advice is the most dangerous kind.** "Soft-gate the
   ...
   evidence it is permitted here â€” and the more routine it sounds, the less
...
defect is not that it is absent â€” it is that it cannot convert:
...
- `components/ShareLifeCard.tsx` (417 lines, added in PR #67) is **imported by
  nothing** â€” dead code. It is also text-only, and `require`s
...
invisible to a plan written from the outside â€” and are worth more than the
plan's top three items combined.

## 2026-08-09 (same day) â€” two stale-ref bugs that only a batch could expose
...
rapid taps â€” not batch-only artifacts:
...
then derived from committed state â€” `weeksAdvanced` is
...
commits. Assert on committed state *after* the block instead â€” which is what
...
## 2026-08-09 (same day) â€” I counted registrations and called it a tab bar
...
Five of the nine carry `href: null` â€” deliberately, with a comment three lines
...
per-stat contributors and advice in the rest â€” a modest-benefit refactor, not
...
2. **Read the comment next to the thing you are grading.** Both errors were
   pre-refuted in the source. The repo's own rule â€” *don't trust an audit claim
   that "file:line is broken" without re-reading the source* â€” applies to audits
...
a group as a tab BY DEFAULT â€” so a new screen silently joins the bar and a
...
## 2026-08-10 â€” Two content-quality goals set, both wrong. Check a goal is reachable before you ship it.

`scripts/lib/contentQualityRatchet.js` has now had a target retired twice in two
days, by me, in consecutive commits. The floors were fine both times â€” the
regression protection was never the problem. The *ambitions* were fiction.

**Goal 1: `medianAbsHappiness` â†’ 15.** Retired within hours. It measured
...
**Goal 2: `soloHappinessMedian` â†’ 10.** Its replacement, set the same day, and
wrong in two independent ways:

- **Unreachable by the work it implied.** Simulating a perfect pass â€” every
  trivial happiness-only outcome raised to 10, excluding the file that documents
  itself as deliberate flavour â€” lands the median at **8**, because the
...
- **Aimed at a population that was already correct.** Of the 46 trivial
  ...
  flavour file. Declining an offer *should* do almost nothing â€” that is what
...
**Two, out of 235** â€” and both are in the flavour file. The corpus never had the
...
2. **A goal you can't reach is worse than no goal.** It is a permanent red mark,
   and a gate nobody can satisfy trains people to skim it â€” the same failure as
...
alone â€” a change that alters no number is churn, and it would have looked like
fiddling with a metric to pass it.

---

## 2026-08-10 â€” "Preflight green" was read off a banner, not an exit code

I reported `npm run preflight` as **âœ… ALL PASSED** while the command was
exiting non-zero. Both statements were about the same run.

`scripts/preflight-check.js` prints its own `PREFLIGHT CHECK SUMMARY` banner â€”
...
1. **A gate's verdict is its exit code.** Any sub-step can print a cheerful
   banner. `echo "EXIT: $?"`, or read the last line of the chain â€” never a
...
opposite of what a 1 270-warning count suggests. One line â€”
`const AUTO_REST_TARGET_ENERGY = 70;` sitting between two `import` statements in
`GameActionsContext.tsx` â€” made all 103 imports below it "in body of module" to
...
statement is in the wrong place.** Warning counts are not linear in effort â€”
`import/first` and friends multiply one mistake by the size of the file.

---

## 2026-08-10 â€” `pgrep -f` / `pkill -f` match the watcher, not just the watched
...
because the shell's command line *contains that string* â€” it is the command
...
The wrapper's own command line contains `capture-story-mode-shots` â€” twice. So
...
2. **"Is it running?" is not a health check â€” "is it producing output?" is.**
...
## 2026-08-10 â€” Story mode's first tap ended in a funeral, and only running it showed that
...
invitations** â€” and the feature's whole premise is that the player takes no
...
"does the batch equal the same number of individual ticks?" â€” which it does,
...
2. **When a feature removes a player's touchpoints, enumerate what those
   touchpoints were doing.** Weekly taps were not just pacing â€” they were
...
- `lastTickOutcomeRef` had no `advanced` field, yet TWO comments inside
  `nextWeek` reasoned about it â€” "`advanced` stays false, so `liveYear` must
...
the loop reduced to one call â€” and the end-to-end behaviour verified in a real
browser, which is the only place it is observable.

---

## 2026-08-10 â€” Three rounds of screenshots against a bundle I had already replaced
...
times, and stared at screenshots wondering why my copy change was not in them â€”
...
JavaScript. Every downstream observation â€” the picker copy unchanged, the
heading still sliced â€” was real, and every diagnosis I formed from it was wrong.
...
1. **Verify the ARTIFACT, not the port.** "Something is listening" and "the
   ...
   filenames make the second one a one-line check â€” use it before every capture
...
## 2026-08-10 â€” A verification step is code, and mine could only ever fail
...
events were dispatched â€” the same "is it running?" mistake recorded twice above.
...
that is never present â€” **a check with no passing branch.**
...
1. **A check that cannot pass is worse than no check.** Before trusting a new
   ...
   succeed even once â€” that alone should have disqualified it.
...
## 2026-08-10 â€” A gate on `weeksLived` that never let anything through
...
`weeksLived` is the ABSOLUTE life counter (CLAUDE.md Â§4.2). A character created
...
1, 2, 8, 9` â€” the numbers the implementation assumed. A test suite written from
...
I read that file during this work â€” it is quoted in the coach's own header â€”
...
1. **Never compare `weeksLived` to a small number.** It is an absolute clock,
   ...
   baseline and a subtraction. Â§4.2 says this; it earns a third entry here.
...
4. **"It works in the test" is not "it works."** Nothing here was verified by
   running the app until it had already been declared done â€” twice.

---

## 2026-08-11 â€” A clean audit is not a clean app
...
away â€” including the game's only outright money printer (a dark-web hack that
...
already names as the most repeated here (Â§4.4 gateâ†’grant), each narrowing the
next:

1. every `if (â€¦ < cost)` followed by a `setGameState` â†’ 50 candidates, mostly
   noise (display code, documented fast paths);
2. every hand-written `money:` write inside an updater â†’ 21, of which 16 were
   debug/simulation tooling;
3. of the rest, the ones whose updater had NO `return prev` refusal â†’ 5, of
   which 3 were real.
...
Every one of the three had the same root â€” the only affordability check lived
outside the updater â€” but the *symptom* was set by the write site's clamp:

- `sanitizeAmount` (money â‰¤ 0 â†’ 0) **forgave the debt and granted anyway** â†’
...
none acted on it. The code that did expire it sat in `processVehicleWeekly` â€”
the pre-WeekContext ancestor of the live reducer â€” which has **no production
...
helpers â€” immunity, luxury risk and patent income were all flagged and all
...
been misread in this repo â€” the first was reading its printed banner. Echo
`$?` immediately, on its own.

**A new check must be tested against correct code, not just broken code.** The
first version of the G5 gateâ†’grant analyzer hard-coded `return prev` and so
reported every `return prevState;` refusal as a bug â€” three false positives out
...
### Addendum â€” what the bot review of that audit caught
...
double-upgrade â€” but the return value was still built from the caller's
...
That shape carries the Â§4.1 hazard (React only evaluates an updater eagerly when
...
recoverable â€” the player taps again, state was correct throughout. The opposite
...
did not change the count â€” nothing was hiding there â€” but the count had been
...
time â€” documented behaviour â€” so the key ships inside the app, and the same key
...
Nothing to change in the code today â€” single-player, save on device, a player
...
## 2026-08-11 â€” Weekly audit: a false red from a cold container, and the coach's second blind spot
...
directly. Run immediately after cloning â€” before `npm install` finishes â€” it
reported "Test-tree type errors ROSE to 2" and flipped the whole domain to
ðŸŸ¡. `npm run type-check:tests:ratchet` said **0**; re-running the audit after
...
usually just missing dependencies") â€” this is the same trap wearing the
...
formal career or political salary (`applyLifetimeStatistics.ts:93-94,151` â€”
...
they are told to "find their first job" for up to eight weeks â€” the exact
...
done X" signal, check every income/progress source that should count â€” a
...
## 2026-08-11 â€” BBQ bug report: a writer with no caller, and two findings I over-graded
...
already-owned and insufficient-funds correctly â€” and had zero call sites in
...
(`__tests__/economy/crimeToolsReachable.test.ts`) asserts the whole CHAIN â€” job
requires id â†’ id exists in catalogue â†’ catalogue has a screen â†’ that screen
calls the action â€” because any single link passing proves nothing.

**Two of my own findings were over-graded, and re-reading the source before
writing the fix is what caught both.** CLAUDE.md Â§8 says not to trust a finding
...
- I claimed `calculateNetWorth` ignores bank accounts while prestige counts
  ...
  defect was three *display* surfaces â€” narrower, and still worth fixing.
- I graded the flagged-vendor sit-out a P0 bug and proposed removing it. It is
  ...
  one layer up â€” the scam odds are computed and never shown, and rep is a
...
assumed. Grep the tests for the behaviour before calling it a bug â€” if the
...
`src.indexOf('openAccount')` â€” matching an *import* â€” then read a fixed 6,000
...
inside the updater â€” textbook gate-then-grant. It had never mattered, because
...
a call to `applyMoneyDelta`, which does not refuse an over-ceiling credit â€” it
...
anti-bigamy check refuses a second partner â€” right, and it stays. But
...
`meet_friends` want that lists `'friend'` among its target types â€” a feature
...
`promoteMatchToFriend` took the C-9 count 62 â†’ 63. My first instinct was to
...
ones. Two rules in one file, each scoped to a different situation â€” and taking
...
in the suite asserted on committed STATE rather than the return value â€” so none
of them would have caught it.

**Deleting a dead field is how you find out who was writing it.** Removing
`Company.money` â€” a field nothing had ever read â€” immediately broke four TEST
...
said 5 Ã— 3 = 15. Both were internally consistent, both were readable, and the
300-gold promise was impossible for as long as anyone cared to check. Retarget
the achievement, but the actual fix is the test that ties them together â€”
...
`(24 / 4) / 200` â€” three percent of weekly income, for a seven-figure price â€”
...
## 2026-08-11 â€” A total and its itemisation are one fact written twice
...
was forgotten in the second â€” stocks, then luxury, then (in the very commit
...
fix was to delete the second list â€” `computeNetWorth` now reports what each
...
And prefer the pure module over the component when the invariant needs a test â€”
the render harness cannot seed the state that makes the disagreement visible.

## 2026-08-11 â€” Reconciling an automated fixer's patch with your own
...
the R-1 money-creation shape in mirror image â€” benign today only because an
...
functional `setAmount(prev => â€¦)` form and guarded `max <= 0`; mine read
`amount` from the closure. It also caught a third display site I had missed, and
added a sanity ceiling on acquisition revenue â€” `isFinite` rejects `NaN` and
...
were free to diverge again â€” and by the end of its own patch they had (the
...
right without ego â€” the stale-closure fix was better than mine. And check
...
## 2026-08-12 â€” Wiring up a dead feature promotes its dead ends too
...
and no state â€” a brand-new instance of the defect the whole audit was about,
...
assert the mapping in BOTH directions â€” every consumer kind has a producer, and
...
## 2026-08-12 â€” A ratchet anchored on byte distance is not a ratchet
...
fix was to anchor on MEANING â€” everything after the last `setGameState(` call,
...
regression â€” but say so explicitly at the constant, or the next reader reads a
raised ratchet as someone getting a build unstuck.

## 2026-08-12 â€” The canonical context file can lie, and it lies loudest

`CLAUDE.md` Â§7's v38 entry described story mode as a live feature: a batching
interaction, `resolveGameMode` in `lib/gameMode/mode.ts`, and
`__tests__/gameMode/batchEquivalence.test.ts` â€” "the test to run before touching
...
That file opens with "Canonical project context for AI assistantsâ€¦ this document
...
grep the docs for it in the same change â€” `CLAUDE.md`, `DEV.md`, `WORKFLOW.md`
...
or read by the next engineer â€” both act on it.

---

## 2026-08-12 â€” Two declarations of one type kept a whole mechanic dead
...
**The pattern â€” and it is the interesting part.** The reason nobody wired it up
...
duplicate type didn't just permit the drift â€” it made the correct fix look
...
## 2026-08-13 â€” The explainer that didn't equal the thing it explained (weekly audit)

**Symptom.** The Net Worth card (`IdentityCard`) shows the canonical `netWorth()`
(`lib/progress/achievements.ts`) â€” the figure prestige, achievements and the
leaderboard all use. Tapping it opened `NetWorthBreakdownModal`, whose total came
from a *second* engine (`buildNetWorthItemisation` â†’ `computeNetWorth`,
...
bank and laundered-BTC rows and its doc comments *claimed* parity â€” but claiming
...
and reading the exact residual â€” 2 basic miners (2Ã—2500) + a laptop (1200) = 6200,
...
the rows I can see add up" â€” the latter passed the whole time the headline was
wrong.

**The fix, and where its blast radius stops.** Fixes 1â€“3 are unambiguous and live
...
`netWorth()` â€” the one prestige/achievements/leaderboard read â€” is **untouched**,
so the change cannot move a prestige gate. The miners/items scope gap (#4) is a
genuine design call â€” *should* owned hardware and inventory count toward net worth?
â€” that belongs to the owner, not to a modal deciding on its own, so it is filed,
...
miners+items residual to the exact dollar â€” so a re-introduced fee or a dropped
...
## 2026-08-13 â€” A whitelist merge on load ate every "carve-out" field
...
compiles cleanly and the only symptom is a wrong face. (This one was mine â€”
...
`initialGameState`** â€” and a field whose stored default is `undefined` is, by
the Â§7 carve-out rule, deliberately NOT written into `initialGameState` at all.
...
(v28) was the expensive one â€” it exists *because* the module-level boolean it
...
- A merge that iterates the DEFAULTS is a whitelist. If the schema has optional
  fields, iterate the union â€” the saved object's own keys have to survive too.
...
- "No backfill needed, absence already resolves" is a claim about the SAVE
  FORMAT. It says nothing about whether the key survives a round trip, and Â§7
...
## 2026-08-13 â€” A gate can be a deadlock, and "monotonic" is a claim you have to test
...
error logs. Nothing had crashed â€” the save was simply parked at unlock tier 2
...
1. **The progression spine was circular.** Chapter 3's goal is "buy your first
   stock or property" while `app:stocks` and `app:realestate` were tier 3 â€”
...
2. **The one escape route slid backwards.** `unlockTier`'s milestone fallback
   ...
   manages it. The file's own header said "NOTHING IS EVER TAKEN AWAY â€”
...
- **A comment asserting an invariant is a TODO for a test.** Rule 2 had been
  ...
  wide â€” `unlockTier(after) >= unlockTier(before)` across a spend â€” and nobody
...
- **Derived-not-stored does not mean derived-from-anything.** Deriving unlock
  ...
  and already persisted â€” `lifetimeStatistics.peakNetWorth`, maintained every
...
- **A clean validation report is a finding, not a dead end.** Valid save, no
  errors, no warnings â€” which ruled out corruption and pointed straight at
  logic that was working exactly as written.

---

## 2026-08-14 â€” A clock-farm fix that closes one faucet has siblings
...
guards, because those two only refuse a REWOUND clock â€” advancing the device
...
The `gameWeek` gate was already a parameter of that shared helper â€” the fix had
...
- The fix is per-tier where the GRACE is per-tier. The free tier has no
  ...
  decision, not a bug fix â€” left to the owner, not changed unattended. Correct
  fixes still respect a documented product choice; flag it, don't overwrite it.
- Â§7 has drifted in practice. `lastAdCashBonusWeek` shipped as an
  undefined-default settings marker with a comment asserting "no migration or
  STATE_VERSION bump is owed" â€” which contradicts Â§7's "still bump the version"
...
- What went wrong: the Android Play Store local build (run #5) failed in two
  ...
  that cannot happen. Nothing in the repo could have made that run pass â€” only
...
- Pattern: a fail-fast guard reports as a red job identical in shape to a real
  ...
  whether the missing input is something the repo can even supply â€” a repo secret
...
- Second finding, the one that mattered: that same run was dispatched with
  ...
  Â§9 documents â€” 1.4.x is the App Store Connect version RECORD (what users see on
  the store page), a deliberately different number from the binary version â€” but
  Â§9 only explains the split, it never stopped the wrong number being typed into
...
- Third finding: both local workflows carried comments asserting `eas.json` sets
  ...
  no such commit) â€” it is `"remote"`. The 2026-06-11 lesson below prescribes
...
### 2026-08-14 â€” "Can't access apps": a high-water mark that only the week tick ever stamped
...
- What happened: a player reported "Can't access apps, can't claim reward" with
  ...
  in between, and in the later grid exactly two apps are open â€” Contacts and
...
- Root cause: `wealthMark` is `Math.max(liquid, liveNetWorth, peakNetWorth)`.
  ...
  non-monotonic ones is not monotonic** â€” whenever a live term is the maximum,
  ...
  never sampled at all â€” and early play is mostly that kind of money.
- Pattern, and the reason this is the SECOND report of the same bug (2026-08-13,
  ...
  property â€” the suite checked tiers at fixed states, which every version of this
  code passes.
- Rule: when a value is documented as monotonic, test it as a WALK â€” a sequence
  of increases and decreases, asserting the derived quantity never decreases â€”
...
- Where the fix went, and why not MoneyActions: CLAUDE.md Â§4.4 makes
  `updateMoney`/`applyMoneyDelta`/`batchUpdateMoney` the documented way to move
  money, which makes them look like the choke point. They are not â€” `buyItem`,
  `sellItem` and many other actions write `stats.money` inside their own updater,
  correctly, for the atomicity Â§4.4 itself demands. The purchase in this very bug
...
- Second, smaller finding, and the other half of the report: `LifeChapterCard`'s
  completed state rendered a full-width solid-amber bar with bold dark text â€”
  pixel-for-pixel the app's primary CTA â€” on a `View` with no handler. The
...
### 2026-08-14 (later) â€” auditing the fix: the sibling card, and the exploit the fix created
...
from a bug report. `AmbitionCard` â€” directly below it on the same screen,
carrying the largest reward in the game â€” had the identical solid-amber
...
monotonic â€” but `byMilestone`'s tier-1 condition also read `state.currentJob`,
...
tier 1 â†’ 0 and lost the Progression tab, Contacts and Bank.
...
**3. The fix created an exploit â€” and half-fixing it was worse than not.**
...
read the raw balance â€” and a test written to assert the intended behaviour
...
- Pattern: a "sticky" mechanism makes every pre-existing inflation of the
  ...
  transient bugs into permanent ones â€” the audit question is not "is this value
...
and a network-favour introduction needs a `business` contact â€”
...
chapter â€” rule 3, the same trap a player was stranded in on 2026-08-13. The
...
- Rule: when a permissive-looking check turns out to be load-bearing, the
  deliverable is the guard, not the change â€” a comment at the site and a test
...
â€” an owner call, not an audit call.

### 2026-08-14 (round 2) â€” three scripted sweeps, and the review that cleared a dead field
...
effect is handled in the week loop and stress-tested â€” and it could never fire,
...
- Rule: for any field that gates content, check the writer before the reader. A
  gate with no writer is not a subtle bug â€” it is a feature that does not exist,
...
literal â€” and `lib/realEstate/rentals.ts` cited the CONSTANT by name in its own
...
- Rule: a test that pins a literal (`toMatch(/>= 4/)`) blocks the fix and proves
  nothing about the named copy. Pin BOTH â€” that the code reads the constant, and
...
something bad, and its money arm read `bankruptcyTriggered` â€” which nothing
...
## 2026-08-14 â€” A screenshot tool that fails silently is worse than one that crashes
...
reworded â€” and when it goes stale it does not stop. A missed label just means a
...
1. It waited for `New Game` on the main menu â€” a label that only exists once a
...
3. It matched the market's Computer row on `$5000`, the item's BASE price,
   while the market applies inflation â€” the card reads $5,300 by then. So the
...
4. **`page.mouse.wheel` does nothing at all** on react-native-web's ScrollView â€”
...
The scrolling that DID happen was accidental â€” `clickText` calls
...
- Pattern: a property that does not exist evaluates to `undefined`, and
  ...
  an error â€” it produces silence.
...
- Rule: clearing an `as any`/`require()` backlog is bug-hunting, not tidying.
  Budget for the bugs and write the test â€” the conversion is the cheap part.
...
- Rule: `import type` is ERASED by tsc and emits no runtime require, so it
  ...
  imports reports cycles that do not exist â€” mine reported four, all routed
...
- Rule: a lazy require defers module EVALUATION, not just typing. Before making
  one eager, check the target for top-level side effects â€” that half is
...
1. **Narrow deps on a helper call** â€” `useMemo(() => activeLegacyBuffs(gameState),
   [gameState.legacyBuffs, gameState.weeksLived])`. The rule cannot see inside
   `activeLegacyBuffs`, whose signature is literally
   `Pick<GameState, 'legacyBuffs' | 'weeksLived'>` â€” the deps are provably
   exactly right. Adding `gameState` would recompute on every tick and every
   money change, which is the perf regression CLAUDE.md Â§4.1 documents.
2. **The `?? []` alias** â€” `const holdings = stocks?.holdings ?? []`, with the
...
3. **Stable-by-construction values** â€” `useRef`, `Animated.Value`, `setGameState`
   from `useState`. React guarantees the identity; the rule does not model that.
4. **Genuinely fixable (1 of 98, and it was worth 4 warnings)** â€” `datingDeps`,
...
- Rule: a count in a comment is a measurement with a date, not a fact. Say when
  ...
  header claiming 102 and this entry claiming 98, with the truth at 94 â€” caught
...
- Pattern: the same one as Â§6. An unverified characterisation in a comment
...
- Rule: when a rule is wrong for the codebase, write that down NEXT TO THE
  COUNT â€” not in a commit message. The next person to see "98 warnings" will
  reach for the autofix.

## 2026-08-15 â€” The unsound workaround finally shipped, and the file predicting it was already in the repo
...
family business and got an error banner reading
    ...
    Need $10,000 for "marketing" â€” you have $40.25M.

Four thousand times the money, told they were short. The money was fine â€” the
...
affordability message â€” a branch reachable ONLY this way, which is why the
...
- Rule: the shape was already documented as unsound, by name, with a measurement,
  ...
  succeeded") and in CLAUDE.md Â§4.1. It was still written into new code, because
  the comment above it called it "the repairRig pattern" â€” a name that makes a
...
- Rule: the sound fix is the one both files already prescribed â€” make the
...
- Rule: every action suite in this repo drives `setGameState` with
  ...
  stub more obliging than production turns a suite into a mirror** â€” the
...
- Rule: the C-9 ratchet in `__tests__/refactor/updaterResultRatchet.test.ts` did
  ...
  "the fixed shape" â€” but the fixed shape for the DETECTOR is the unsound shape
  ...
  Each reports a canned failure ("Unable to repair right now", â€¦) for an action
  that worked, whenever its update is not first in its batch. Not fixed here â€”
  this change is scoped to the reported bug â€” but the list is the work, and the
...
- Rule (one more stale claim, same family as Â§6/Â§7 above): the regression test
...
## 2026-08-15 â€” Fixing the class the player report belonged to, and what the ratchet had been hiding
...
- Rule: **a ratchet can hide the thing it counts.** `updaterResultRatchet` had
  ...
  and the file's own header â€” plus `updaterTimingContract.test.tsx`, plus
  `petActionResults.test.ts` â€” all carried the caveat that the shape was
...
- Rule: **two sound fixes, and picking the wrong one costs a lot of diff.**
  ...
  capture gives identical reporting to a full preview/commit resolver â€” and it
...
- Rule: **a test that pins a shape can outlive the shape's justification.** Five
  ...
  further and asserted a *swallowed* updater reports failure â€” a property that
...
- Rule: the accepted trade is worth stating at every site rather than once. A
  ...
  refusal on the common path â€” and a comment at each assertion stops the next
  person "fixing" it back.
- Rule: **a workaround's own documentation is a to-do list.** `breachBrandDeal`
  shipped a "âš ï¸ DO NOT TRUST THE RETURN VALUE" banner and a whole extra pure
...
## 2026-08-15 (round 3) â€” my own detector's zero was the same lie the ratchet had told
...
- Rule: **a detector's zero is only as good as its recall, and a zero is exactly
  ...
  `let totalRewardsOut = 0` read in a template string â€” same defect, different
...
- Rule: **triage heuristics need both error directions checked.** My first sweep
  ...
  three functions with NO outer guard at all â€” they surfaced only from a
...
- Rule: an indentation check is not a scope check. `swipeOnProfile` computes
  `matched` in an ordinary `if` block ABOVE its dispatch â€” correct code â€” and a
...
- Rule: **`void` returns hide refusals completely.** `maintainProperty` returned
  ...
  not afford maintenance tapped the button and got silence â€” no error, no
...
- Rule: when the honest answer is "one left", say WHICH one and pin it by name.
  ...
  asserted by name in the ratchet â€” wiring it into the tick trips that test
  before it can hurt anyone. "Zero except the one we know about" is only a
  useful statement if the exception is written down.

## 2026-08-17 â€” a git pathspec `*` is not a shell glob
...
- Rule: **git pathspec wildcards match across `/`.** The quoting was correct â€”
  the glob had to reach git rather than the shell â€” but git's own fnmatch runs
...
- Rule: it was recoverable only because nothing had been committed yet.
  ...
  and commit last â€” the index is the undo buffer.

## 2026-08-19 â€” the fabricated-property obituary bug, a third time
...
`RealEstate` has no `value` field â€” market value is `currentValue` (falling back
to `price`) â€” so every property evaluated to `undefined ?? 0`, the whole asset
...
- Rule: **the `(r: any)` cast is the whole bug, again.** It let a fabricated
  property name compile and read `undefined` inside a falsy-coalescing gate â€”
  exactly what CLAUDE.md Â§5 says clearing `as any` keeps catching. Static audits
  are blind to it because the code type-checks; only reading the value against
  the canonical reader (`lib/progress/achievements.ts` â€” `currentValue ?? price`,
...
- Rule: when the static layer is fully green, the qualitative pass is where the
  ...
  grandchildren v34) returned all-clean EXCEPT this â€” and it is a player-facing
  correctness bug in the share text, the cheapest acquisition channel.

## 2026-08-19 â€” 22 minutes and 28 log rows: a step that was blocked, not slow

"Why is it taking 22+ min and it just 28 rows in?" â€” `Submit to TestFlight` in
`eas-build-local-ios.yml`, sitting on `- Submitting` with no output.

Reading the rows is the whole diagnosis. Rows 10â€“25 are the real work: the .ipa
...
it is `eas submit`'s built-in wait â€” EAS queueing a submission worker, then that
...
wedged job produce byte-identical logs â€” there is no way to tell "working" from
...
App Store Connect upload â€” and `wait_for_submission: false` ends the run at
...
- Rule: **an unbounded wait needs a heartbeat, not just a timeout.** A timeout
  ...
  move â€” cancel and rebuild â€” is the expensive wrong answer, because a rebuild
...
- Rule: a custom `if:` on a GitHub Actions step **replaces** the implicit
  ...
  submission and reported its outcome as this one's. Write `success() && â€¦`.
...
- Rule: **a comment saying "these two steps must stay together" is not a
  guarantee â€” pin it.** `--no-wait` is safe only because a watch step follows
  ...
  verified by breaking each invariant and watching it go red â€” a guard nobody
...
- Rule: **say what a success does NOT mean.** EAS reports `FINISHED` when the
  ...
  (CLAUDE.md Â§9). A green step reading as "shipped" is how a build sits in
...
## 2026-08-19 â€” Beta Hub: what building the recruitment funnel taught
...
- Rule: **a derived badge must be derived from the holder, not from the crowd.**
  ...
  people act?*
- Rule: **the same gate-then-grant rule as Â§4.4 applies to a web backend, and
  the enforcement point is the database.** Idea voting checks "have you already
  voted" and increments a tally â€” the exact double-tap shape that has shipped
...
- Rule: **an allow-list is only proven by trying to get past it.** The admin
  ...
  obviously safe â€” so the smoke test POSTed `{"admin_token":"HACK"}` through it
...
- Rule: **when the sandbox cannot reach the deployment, find another caller.**
  ...
  `create extension http` let every route be exercised from inside SQL â€” signup,
  tester token, progress, feedback, bug, idea, vote, all seven admin routes, CSV
  export, the 401/404 paths â€” then the extension was dropped and the test rows
  ...
  call that follows it â€” the function connects in a different session and reads
  the pre-update value. That looked exactly like a waitlist bug for a minute.
  Commit first, then call.

## 2026-08-19 â€” A goal you cannot see yourself approaching is not a goal
...
pinned at zero. It never lies â€” it just never moves.
...
measure is wrong â€” measure the thing the player is actually doing (applications
sent, relationship score, arrears covered), not the binary outcome that ends the
goal.

Writing the test first surfaced five more goals whose progress was pinned â€” all
...
## 2026-08-19 â€” Research the offer mechanism before designing the offer UI

A rotating IAP sale reads like a UI problem. It is not. Apple's Promotional
Offers API â€” the thing every "how to run a sale" article points at â€” covers
...
else â€” there is no "is this on sale" flag. So a discount badge has to be
...
## 2026-08-19 â€” A unit test can prove a sink works while the wiring above it is dead

`__tests__/services/analyticsFanout.test.ts` proved, correctly and in detail,
that `track()` forwards to Firebase with `telemetry: false` â€” two independent
...
every custom event was dropped at the first branch of `track()` â€” Firebase
...
own documentation â€” the comment explaining why `track('session_start', â€¦)` is
...
## 2026-08-19 â€” A priority constant with no claimant is a lie the compiler cannot catch
...
their own file while every surface in a DIFFERENT file stayed blind to them â€” so
...
implementation.** A registry, a priority enum, a capability list â€” each entry is
...
1. **Three of my four hypotheses were wrong**, and each would have been a
   ...
   and is opened only by a tap. `CureSuccessModal` looked like dead code â€” a grep
   for its flag hit only test files â€” and is set at three sites in
   `ItemActionsContext` under a different spelling. CLAUDE.md Â§8 already says not
...
## 2026-08-21 â€” A blocking popup with no scroll surface is a soft lock, not an overflow
...
it â€” crest, congratulation line, celebration box, three reward rows, closing
line, CTA â€” measures taller than that bound on an ordinary phone, and there was
...
1. **`flexShrink: 1`, never `flex: 1`.** `flex: 1` is flexBasis 0 + grow with
   ...
   and the scroll area resolves to zero height â€” the same bug wearing a
...
3. **Pin the CTA â€” unless the CTA is a list.** `LifeMomentModal` has no single
...
and `CommunityRewardPopup` lack a scroller too and are fine â€” their content is
...
reason `applyCardModalScroll.test.ts` gives â€” reproducing the overflow needs a
...
## 2026-08-22 â€” Three screens, one salary, three numbers

Reported with three screenshots of the same Surgical Director in the same save:
promotion modal **$26K/wk**, work-tab job card **$13000/wk**, Cash Flow â†’ Income
...
`applyCareerSalaryAndPenalty` multiplies it by a stack â€” negotiated raise
premium, the Work Pay Boost gold upgrade, the workBoost IAP perk, the
Negotiation/Executive life skills, the DeepLife+ income boost â€” of which the
...
1. **Extracting the arithmetic is not the fix; making the PAYER call it is.**
   ...
   it â€” the divergence just moved up one layer to the multipliers nobody
...
2. **Pinning it needs a test that compares the two sides, not two constants.**
   ...
   `careerSalary` â€” so re-tuning a multiplier cannot split them apart again.
...
A President's Job Income line said **$100,000/wk** instead of $1,923 â€” and
...
## 2026-08-22 (b) â€” A hand-written mirror of a catalog, and a wrong answer given confidently
...
and it is tested. I asserted the gap from one true premise â€” ladders are
persisted in the save â€” without checking for the mechanism that closes it. The
...
`ceo Â· research_scientist Â· creative_director Â· investment_banker Â· surgeon` â€” a
...
rendered **twice** once applied for â€” the player's own entry with their real
...
content â€” nobody reports a career they have never seen.

Also caught, and mine: the "Current Job" hero still read the base salary,
printing `$13,000/wk Â· Lv 5/8 Â· +100%` â€” stating the premium and withholding it
...
## 2026-08-22 (c) â€” The audit found a bigger one than the report did
...
announced itself â€” the player saw two numbers and reported it. Nobody reports a
...
`calcWeeklyPassiveIncome(prev).total` (`applyIncome.ts`). The tidy-looking move â€”
fold luxury into `calcWeeklyPassiveIncome` so there is "one income function" â€”
...
`contexts/` â€” the same relocation the raise-premium and money-delta symbols
...
## 2026-08-21 â€” Subagent sessions can complete with EMPTY payloads and zero work
...
- **The pattern:** an empty agent report is not evidence of done OR of failure â€” only
...
- **The rule:** after every subagent returns, run git status --short and match changed
  ...
  rounds, stop retrying and do the work directly â€” the plan's file-ownership map makes
