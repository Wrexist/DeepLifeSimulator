...
### 2026-07-14 - Weekly audit: an anti-arbitrage cap was set against the WRONG (highest) borrow rate, not the reachable floor

- What went wrong: `SAVINGS_APR_HARD_CAP` was set to 5.5% with a comment asserting it is "strictly below the cheapest borrow rate (private-banking loan floor = PRIVATE_BANKING_APR_CAP = 6%)." But `PRIVATE_BANKING_APR_CAP` is a `Math.min` CEILING, not a floor — it caps how high a rate goes, it does not stop cheaper rates. The truly reachable borrow rates are BELOW the 5.5% deposit cap: `STUDENT_LOAN_APR = 0.045` (fixed 4.5%), and a personal loan floors at `LOAN_APR_FLOOR = 0.025` once politics perks stack `loanInterestReduction: 20` (`quoteLoan`: `0.12 - 0.005(credit) - 0.20 → 2.5%`). `acceptLoan` deposits the principal into a spendable account, so a good-credit player (savings capped at 5.5%) borrows at 2.5–4.5% and parks the proceeds for ~1–3%/yr risk-free carry. The invariant test (`rateEnvironment.test.ts` "no arbitrage") gave false confidence — it only fed `LOAN_BASE_APRS` through the `effectiveLoanAPR` delta dimension, never modelling `politicsAprReduction` or `creditScoreAPRAdjustment`, so it cannot see the 2.5% floor path. Graded MEDIUM (bounded by DTI, `MAX_ACTIVE_LOANS=6`, and the slow spread; and the change actually REDUCED arbitrage from a prior 11.25% deposit rate — an incompletely-closed exploit, not a fresh regression).
- Pattern: an anti-arbitrage guard must be pinned to the MINIMUM reachable rate on the opposing side, computed through the FULL pricing pipeline (every discount/perk/credit adjustment at its most favorable), not to a named "cap"/"base" constant that reads like a floor but isn't. A test that exercises only the base-rate dimension of a multi-factor price will pass while the perked path leaks.
- Rule: set `SAVINGS_APR_HARD_CAP` strictly below the true minimum reachable borrow rate (`< LOAN_APR_FLOOR`, e.g. 0.02) OR raise `LOAN_APR_FLOOR`/`STUDENT_LOAN_APR` above the deposit cap. Rewrite the invariant test to drive the WHOLE `quoteLoan` pipeline (max `aprReduction=0.2`, best credit adjustment, every loan type incl. student) and assert `maxDepositAPY < minLoanAPR` — not just over `effectiveLoanAPR`. When a comment names a constant as the bound, verify that constant is actually the extremum on that side (a `Math.min` ceiling is not a floor).

### 2026-07-14 - Weekly audit: a double-pay fix split one shared income cap into two independent caps, doubling the ceiling

- What went wrong: the mining double-pay fix (warehouse miners removed from the cash passive-income path and minted as crypto in `applyMiningCryptos` instead) correctly stopped the double-credit, but each path now carries its OWN `$100K/week` cap — `MINING_INCOME_CAP` in `passiveIncome.ts` (company miners → cash) and `MINING_USD_CAP` in `applyMiningCryptos.ts` (warehouse miners → crypto). Previously both miner types accumulated into ONE `cryptoMiningIncome` variable clamped by a SINGLE shared `$100K` cap. A player who owns both mining types now has an effective ~$200K/week mining ceiling; the new comment even claims it "keep[s] the same balance the old cash path enforced," but the old cap was shared across both sources, so the aggregate ceiling silently doubled. Graded MEDIUM (each mining type is separately gated behind large capital; most players never approach either cap).
- Pattern: when you split one accrual path into two (to fix a double-count), any GLOBAL limit that used to span both halves must be split into two limits that SUM to the original — not duplicated wholesale onto each half. Duplicating the cap constant preserves the per-source number while doubling the aggregate.
- Rule: when refactoring a capped aggregate into per-source paths, make the caps share one budget — thread the first path's realized total into the second and clamp to `max(0, TOTAL_CAP - alreadyEarned)` (or compute both, then scale to a single combined ceiling). Add a test that maxes out BOTH sources and asserts the combined weekly credit ≤ the original single cap.
