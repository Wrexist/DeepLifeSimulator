...
- Fix: added a MONOTONIC epoch high-water mark `settings.deepLifePlusLastGemClaimAt` (optional, undefined-default → no version bump, matches the wave's own pattern). `canClaimDailyGems(state, todayKey, nowMs?)` now also rejects a claim whose `nowMs` is below the stored mark (minus a 5-min NTP skew tolerance); `claimDailyGems` stamps `max(previous, now)` so the mark never decreases. This fully closes the backward-clock reclaim, and turns the forward-clock farm into a self-punishing move (jumping the clock ahead advances the mark, so the cheater is locked out until REAL time catches up to the furthest point they jumped — they can never return to real-time play and keep claiming). `nowMs` is optional so legacy 2-arg callers still work (guard skipped); the one real call site (`DailyGemClaim.tsx`) passes `Date.now()`, and the CTA is hidden while the clock is rewound so it never shows a button that silently no-ops.
- Pattern: a client-only game (no server clock — which is exactly why save integrity leans on HMAC tamper-detection) cannot FULLY stop an offline clock-forward cheat, but "gate a real-time reward on a bare wall-clock day-string" is the weak form that's trivially farmable in BOTH directions; a persisted monotonic high-water mark is the strongest client-side bound and closes the backward vector outright + makes the forward vector self-limiting, with zero UX cost to a legit midnight-crossing player (their `now` is always well ahead of the mark). The static economy audit checks constant LADDERS (APR ordering, tax monotonicity, miner prices) but has no notion of a real-time/daily claim surface — this class only surfaces under the exploit-lens qualitative pass.
- Rule: any real-time/daily/cooldown reward (especially one paying premium currency) must persist an absolute monotonic epoch high-water mark and reject claims whose `now` is below it — never gate solely on a clock-derived day/hour STRING. Audit-tooling upgrade worth doing: extend `audit-economy.cjs` to flag any reducer that reads `utcDayKey`/`new Date()`/`Date.now()` for claim ELIGIBILITY without also comparing against a persisted `*At`/`*Epoch` monotonic field, so "clock-string-only daily claim" becomes a named static finding instead of a subagent catch. Lens reaffirmed (3rd audit running): the save lens caught the last two weeks' real bugs, but this week the economy/exploit lens did — run all five every time; the clean lenses are not the ones that will bite.
