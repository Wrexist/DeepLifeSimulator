...
### 2026-07-15 - Weekly audit: new fields shipped AFTER a version bump inherit no migration/repair backstop

- What went wrong: the big content batch (Luxury `luxuryItems`, Life Ambitions `ambitionId`/`ambitionCompletedMilestones`/`ambitionRewardClaimed`) was added to `initialState.ts` in commits that landed *after* `STATE_VERSION` was already bumped to 22 by an earlier Wave-A commit — and those content commits did NOT bump the version again. Consequence: `runMigrations` early-returns when `currentVersion === CURRENT_STATE_VERSION`, so a save written by the intermediate v22 build (or any prior save laddered to 22) never runs a step that fills these fields, and `repairGameState` doesn't backfill them either. No crash today only because every consumer is null-safe by construction (`?? []`, `|| []`, and helpers that accept `undefined | null`). It is pure latent fragility: the first future reader that writes `state.luxuryItems.map(...)` or `.length` unguarded will crash on every pre-existing save, with no migration or repair to catch it.
- Pattern: STATE_VERSION is a per-release token, but content lands in many commits across the week. Only the *first* schema-touching commit tends to bump the version; later same-week commits that add fields ride the already-bumped number and silently get zero save-integrity coverage. The migration/repair safety net is keyed on the version *changing*, so any field added without its own bump is invisible to it. The static audit passes because it only checks that [2..N] has no gap and that STATE_VERSION is consistent — it does not diff `initialState` fields against migration/repair coverage.
- Rule: any commit that adds a field to `initialState` must EITHER bump `STATE_VERSION` with a migration step that fills it, OR add a `repairGameState` default-fill — presence must be guaranteed by the load path, never left to call-site guards alone. Prefer the `repairGameState` backfill for additive-optional fields (it runs on every load regardless of version, so it also covers same-version saves the migration ladder skips). For the audit itself: add a static check that greps every top-level `initialState` field and asserts it appears in either a migration body or `repairGameState` — turn "conventionally guarded" into "provably present." A clean money/crash pass does not imply save-safety; the save lens must diff the schema, not just the version number.
