...
### 2026-06-22 - Weekly audit `--full` reports a false 🟠 perf FAIL when node_modules is absent

- What went wrong: The scheduled weekly-audit routine ran in a fresh container with **no `node_modules`** (deps never installed). `npm run audit:weekly` (pure `.cjs` static scripts) passed, but `audit:weekly:full` ran the perf jest suite, which died with `Preset ts-jest not found` and was reported as a 🟠 high "Performance jest suite failed — see CI logs." After `npm install`, the same suite passed (perf 4/4, money-conservation 4/4, economy 29, save-migration/longrun 40, type-check clean). The "high" was a pure environment artifact, not a code regression.
- Pattern: the dynamic backstop can't distinguish "tests failed" from "test runner couldn't start (missing deps)" — both surface as a blocking FAIL. A scheduled routine on an ephemeral container will hit this every run unless deps are guaranteed installed first.
- Rule: before trusting any 🔴/🟠 from `audit:weekly:full`, confirm `node_modules/.bin/jest` exists; if not, `npm ci` (or `npm install`) and re-run. The repo needs a committed SessionStart hook / setup step that installs deps so the routine's dynamic layer is reliable. A jest "FAIL" whose log is `Preset ... not found` / `Cannot find module` is an env failure to fix in setup, not a code finding.

### 2026-06-22 - Project subagents & prompt templates documented in CLAUDE.md don't exist in the repo (`.claude/` is gitignored)

- What went wrong: CLAUDE.md and `.agents/skills/weekly-audit/SKILL.md` instruct the routine to launch the **Game State Reviewer** (`.claude/agents/game-state-reviewer.md`) and **Save System Auditor** (`.claude/agents/save-system-auditor.md`) subagents and use `.claude/prompts/{crash,exploit}-audit.md`. None of those files exist — the entire `.claude/` tree is **gitignored** (`git check-ignore .claude/agents` → ignored). Only the slash-command skills were migrated to the tracked `.agents/skills/` dir; the subagents, prompt templates, and `.claude/settings.json` hooks were not. The deep qualitative pass had to be run via generic general-purpose agents fed the trap checklist instead.
- Pattern: tooling referenced by absolute `.claude/...` paths in tracked docs silently breaks once `.claude/` is gitignored — the docs read as authoritative but point at nothing in a fresh clone.
- Rule: project subagents/prompts that the routine depends on must live under the **tracked** `.agents/` tree (e.g. `.agents/agents/*.md`), and CLAUDE.md references must point there. Either migrate the two role files + two prompt templates into `.agents/`, or mark them in CLAUDE.md as intentionally-local. Don't un-gitignore `.claude/` wholesale — it also holds local `settings.json`.
