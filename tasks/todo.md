...
- [x] **`session_end` on background — DONE (2026-06-23).** `AnalyticsTracker.tsx` now pairs sessions: emits `session_end` (with `durationMs`) when an active session backgrounds and a fresh `session_start` on resume (cold-start `session_start` still owned by `_layout.tsx`). Decision via a pure, unit-tested `nextSessionAction(sessionActive, next)` that tracks session-active state rather than raw prev AppState — correct across iOS's `background → inactive → active` resume hop. `__tests__/analytics/sessionBoundaries.test.ts` (6/6); analytics suite 20/20.
- [ ] **(ops/release — not code)** Verify in a release build that the already-wired events land: `session_start` (`_layout.tsx:1119`), `week_advanced`/`first_week_completed`/`death`/`prestige` (`AnalyticsTracker.tsx`), `daily_reward_claimed` (`home.tsx:250`), `achievement_unlocked` (`GameActionsContext.tsx:2408`), `ad_shown`/`ad_rewarded` (`AdMobService.ts`), `paywall_viewed` (`SubscriptionModal.tsx:46`), `session_end` (now wired).
...
> **⛔ BLOCKED-BY-ENVIRONMENT (assessed 2026-06-23).** This ticket cannot be safely advanced in a code-only/web session and was intentionally NOT touched:
> - The cited "crash-safe lazy-`require()`" pattern only bundles because its target (`@react-native-async-storage/async-storage`) **is installed**. **Metro resolves `require()` at bundle time**, so a static `require('expo-notifications')` for an *uninstalled* package is a **bundle-breaking error**, not a runtime catch. The stub deliberately contains **zero** such require for this reason. ⇒ The crash-safe wrapper **cannot exist until `expo-notifications` is installed** — there is no safe JS-only slice.
> - Installing the package **+ aligning the `app.config.js` config plugin (Hard Rule #4)** is the exact change that caused the original TurboModule crash; the native module's autolinking/init runs **before JS**, so no lazy-require/try-catch can prevent it. Acceptance explicitly requires verification on a **real iOS 26 build**, which this environment cannot run.
> - **Safe sequence for a build/device session:** (1) `npm install expo-notifications` (one version pinned to Expo SDK); (2) add the config plugin in `app.config.js`; (3) rewrite `utils/notifications.ts` as the crash-safe lazy wrapper, all real calls gated behind `FEATURE_FLAGS.notifications` (already disabled-by-default); (4) wire `utils/smartNotifications.ts` copy → `scheduleNotification`; (5) cap at 3 local types; (6) **build to TestFlight and cold-start a real iOS 26 device** before flipping the flag. Do NOT enable until that cold-start is verified crash-free.
