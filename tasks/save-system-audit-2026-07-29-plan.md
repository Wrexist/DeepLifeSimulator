...
| BRC-4 | Prestige rebuilds the entire state — the most destructive thing a player can do on purpose — and was the one destructive path with NO backup call at all. `before_prestige` was declared, rotation-protected and shown in the restore UI, and nothing ever wrote one. Now snapshotted, with the queued write chained onto it so the copy captures the life being replaced. |
