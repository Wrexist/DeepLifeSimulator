import { CompanyUpgrade } from './types';

/**
 * Canonical company-upgrade catalog — the single source of truth for every
 * company type's upgrades (id, name, description, cost, weeklyIncomeBonus,
 * maxLevel). Previously this table was copy-pasted into createCompany,
 * buyCompanyUpgrade (twice), sellCompany, and the simulator; the duplicates
 * had already drifted into different field projections, so a cost/bonus change
 * had to be made in five places to stay consistent. Import this instead.
 *
 * ## The 2026-08-24 retune (owner-approved balance pass)
 *
 * Every income upgrade used to pay back in ~20 weeks at level 1 (~260%/yr) —
 * an order of magnitude above stocks (7-11%/yr) and net real estate (~5.6%/yr),
 * which made "pour everything into company upgrades" the single dominant
 * strategy in the game. Bonuses now sit at a ~45-week level-1 payback
 * (~115%/yr): still clearly the best ACTIVE path — founding takes capital and
 * education, and the entrepreneur fantasy should out-earn passive index money —
 * but no longer 26x it. Cost x1.5^level and the 10%/level efficiency haircut
 * (floor 50%, company.ts) keep later levels diminishing on top.
 *
 * VALUE-ONLY change: bonuses are baked into `weeklyIncome` at PURCHASE time,
 * so every upgrade already bought keeps every dollar it earns — only new
 * purchases price at this curve (the dark-web vendor-seed precedent).
 * `__tests__/economy/upgradePaybackRatchet.test.ts` pins the band so a new
 * entry cannot quietly reintroduce a 20-week printer.
 */
export const COMPANY_UPGRADES: Record<string, CompanyUpgrade[]> = {
    factory: [
      {
        id: 'machinery',
        name: 'Better Machinery',
        description: 'Increase production efficiency',
        cost: 10000,
        weeklyIncomeBonus: 220,
        level: 0,
        maxLevel: 5,
      },
      {
        id: 'workers',
        name: 'More Workers',
        description: 'Hire additional staff',
        cost: 15000,
        weeklyIncomeBonus: 330,
        level: 0,
        maxLevel: 3,
      },
      {
        id: 'automation',
        name: 'Assembly Line',
        description: 'Automated production line',
        cost: 25000,
        weeklyIncomeBonus: 550,
        level: 0,
        maxLevel: 4,
      },
      {
        id: 'quality_control',
        name: 'Quality Control',
        description: 'Advanced quality assurance',
        cost: 20000,
        weeklyIncomeBonus: 440,
        level: 0,
        maxLevel: 3,
      },
      {
        id: 'warehouse',
        name: 'Smart Warehouse',
        description: 'Automated inventory management',
        cost: 30000,
        weeklyIncomeBonus: 660,
        level: 0,
        maxLevel: 3,
      },
      {
        id: 'safety',
        name: 'Safety Systems',
        description: 'Workplace safety improvements',
        cost: 18000,
        weeklyIncomeBonus: 400,
        level: 0,
        maxLevel: 4,
      },
      {
        // ── Management ──────────────────────────────────────────────────────
        // Pays NO weekly income. Its entire value is reducing Operating
        // Overhead — the passive-income soft cap that silently takes up to 75%
        // of a wealthy player's passive income. Buying management is how the
        // drag becomes a decision instead of a tax; see
        // lib/economy/passiveIncome.ts.
        // NB: 'management' was already taken by realestate's "Property
        // Management" income upgrade. A duplicate id in one array would make
        // buyCompanyUpgrade resolve the wrong entry.
        id: 'ops_management',
        name: 'Professional Management',
        description: 'A real operating team. Reduces the overhead on ALL your passive income.',
        cost: 250000,
        weeklyIncomeBonus: 0,
        level: 0,
        maxLevel: 5,
      },
    ],
    ai: [
      {
        id: 'servers',
        name: 'Better Servers',
        description: 'Upgrade computing power',
        cost: 25000,
        weeklyIncomeBonus: 550,
        level: 0,
        maxLevel: 4,
      },
      {
        id: 'algorithms',
        name: 'Advanced Algorithms',
        description: 'Improve AI capabilities',
        cost: 30000,
        weeklyIncomeBonus: 660,
        level: 0,
        maxLevel: 3,
      },
      {
        id: 'gpu_cluster',
        name: 'GPU Cluster',
        description: 'Faster AI training',
        cost: 50000,
        weeklyIncomeBonus: 1100,
        level: 0,
        maxLevel: 3,
      },
      {
        id: 'data_center',
        name: 'Data Center',
        description: 'Scale operations',
        cost: 75000,
        weeklyIncomeBonus: 1650,
        level: 0,
        maxLevel: 2,
      },
      {
        id: 'ai_researchers',
        name: 'AI Researchers',
        description: 'Cutting-edge research team',
        cost: 40000,
        weeklyIncomeBonus: 880,
        level: 0,
        maxLevel: 4,
      },
      {
        id: 'machine_learning',
        name: 'ML Platform',
        description: 'Machine learning infrastructure',
        cost: 60000,
        weeklyIncomeBonus: 1320,
        level: 0,
        maxLevel: 3,
      },
      {
        // ── Management ──────────────────────────────────────────────────────
        // Pays NO weekly income. Its entire value is reducing Operating
        // Overhead — the passive-income soft cap that silently takes up to 75%
        // of a wealthy player's passive income. Buying management is how the
        // drag becomes a decision instead of a tax; see
        // lib/economy/passiveIncome.ts.
        // NB: 'management' was already taken by realestate's "Property
        // Management" income upgrade. A duplicate id in one array would make
        // buyCompanyUpgrade resolve the wrong entry.
        id: 'ops_management',
        name: 'Professional Management',
        description: 'A real operating team. Reduces the overhead on ALL your passive income.',
        cost: 250000,
        weeklyIncomeBonus: 0,
        level: 0,
        maxLevel: 5,
      },
    ],
    restaurant: [
      {
        id: 'kitchen',
        name: 'Kitchen Upgrade',
        description: 'Modernize kitchen equipment',
        cost: 20000,
        weeklyIncomeBonus: 440,
        level: 0,
        maxLevel: 4,
      },
      {
        id: 'staff',
        name: 'Professional Staff',
        description: 'Hire experienced chefs',
        cost: 18000,
        weeklyIncomeBonus: 400,
        level: 0,
        maxLevel: 3,
      },
      {
        id: 'delivery_service',
        name: 'Delivery Service',
        description: 'Expand customer reach',
        cost: 25000,
        weeklyIncomeBonus: 550,
        level: 0,
        maxLevel: 3,
      },
      {
        id: 'michelin_chef',
        name: 'Michelin Chef',
        description: 'Premium dining experience',
        cost: 40000,
        weeklyIncomeBonus: 880,
        level: 0,
        maxLevel: 2,
      },
      {
        id: 'interior_design',
        name: 'Interior Design',
        description: 'Upscale dining atmosphere',
        cost: 30000,
        weeklyIncomeBonus: 660,
        level: 0,
        maxLevel: 3,
      },
      {
        id: 'wine_cellar',
        name: 'Wine Cellar',
        description: 'Premium wine selection',
        cost: 35000,
        weeklyIncomeBonus: 780,
        level: 0,
        maxLevel: 2,
      },
      {
        // ── Management ──────────────────────────────────────────────────────
        // Pays NO weekly income. Its entire value is reducing Operating
        // Overhead — the passive-income soft cap that silently takes up to 75%
        // of a wealthy player's passive income. Buying management is how the
        // drag becomes a decision instead of a tax; see
        // lib/economy/passiveIncome.ts.
        // NB: 'management' was already taken by realestate's "Property
        // Management" income upgrade. A duplicate id in one array would make
        // buyCompanyUpgrade resolve the wrong entry.
        id: 'ops_management',
        name: 'Professional Management',
        description: 'A real operating team. Reduces the overhead on ALL your passive income.',
        cost: 250000,
        weeklyIncomeBonus: 0,
        level: 0,
        maxLevel: 5,
      },
    ],
    realestate: [
      {
        id: 'properties',
        name: 'More Properties',
        description: 'Expand property portfolio',
        cost: 50000,
        weeklyIncomeBonus: 1100,
        level: 0,
        maxLevel: 5,
      },
      {
        id: 'management',
        name: 'Property Management',
        description: 'Improve property management',
        cost: 30000,
        weeklyIncomeBonus: 660,
        level: 0,
        maxLevel: 3,
      },
      {
        id: 'property_portfolio',
        name: 'Property Portfolio',
        description: 'More rental properties',
        cost: 75000,
        weeklyIncomeBonus: 1650,
        level: 0,
        maxLevel: 4,
      },
      {
        id: 'commercial_real_estate',
        name: 'Commercial Properties',
        description: 'Higher value investments',
        cost: 100000,
        weeklyIncomeBonus: 2200,
        level: 0,
        maxLevel: 3,
      },
      {
        id: 'property_management',
        name: 'Professional Management',
        description: 'Professional management',
        cost: 40000,
        weeklyIncomeBonus: 880,
        level: 0,
        maxLevel: 3,
      },
      {
        id: 'luxury_developments',
        name: 'Luxury Developments',
        description: 'High-end property development',
        cost: 150000,
        weeklyIncomeBonus: 3300,
        level: 0,
        maxLevel: 2,
      },
      {
        // ── Management ──────────────────────────────────────────────────────
        // Pays NO weekly income. Its entire value is reducing Operating
        // Overhead — the passive-income soft cap that silently takes up to 75%
        // of a wealthy player's passive income. Buying management is how the
        // drag becomes a decision instead of a tax; see
        // lib/economy/passiveIncome.ts.
        // NB: 'management' was already taken by realestate's "Property
        // Management" income upgrade. A duplicate id in one array would make
        // buyCompanyUpgrade resolve the wrong entry.
        id: 'ops_management',
        name: 'Professional Management',
        description: 'A real operating team. Reduces the overhead on ALL your passive income.',
        cost: 250000,
        weeklyIncomeBonus: 0,
        level: 0,
        maxLevel: 5,
      },
    ],
    bank: [
      {
        id: 'technology',
        name: 'Banking Technology',
        description: 'Upgrade banking systems',
        cost: 100000,
        weeklyIncomeBonus: 2200,
        level: 0,
        maxLevel: 4,
      },
      {
        id: 'services',
        name: 'Financial Services',
        description: 'Expand financial services',
        cost: 80000,
        weeklyIncomeBonus: 1780,
        level: 0,
        maxLevel: 3,
      },
      {
        id: 'investment_division',
        name: 'Investment Division',
        description: 'Wealth management services',
        cost: 200000,
        weeklyIncomeBonus: 4400,
        level: 0,
        maxLevel: 3,
      },
      {
        id: 'international_banking',
        name: 'International Banking',
        description: 'Global operations',
        cost: 300000,
        weeklyIncomeBonus: 6600,
        level: 0,
        maxLevel: 2,
      },
      {
        id: 'fintech_integration',
        name: 'FinTech Integration',
        description: 'Digital banking solutions',
        cost: 150000,
        weeklyIncomeBonus: 3300,
        level: 0,
        maxLevel: 3,
      },
      {
        id: 'private_banking',
        name: 'Private Banking',
        description: 'Exclusive client services',
        cost: 250000,
        weeklyIncomeBonus: 5500,
        level: 0,
        maxLevel: 2,
      },
      {
        // ── Management ──────────────────────────────────────────────────────
        // Pays NO weekly income. Its entire value is reducing Operating
        // Overhead — the passive-income soft cap that silently takes up to 75%
        // of a wealthy player's passive income. Buying management is how the
        // drag becomes a decision instead of a tax; see
        // lib/economy/passiveIncome.ts.
        // NB: 'management' was already taken by realestate's "Property
        // Management" income upgrade. A duplicate id in one array would make
        // buyCompanyUpgrade resolve the wrong entry.
        id: 'ops_management',
        name: 'Professional Management',
        description: 'A real operating team. Reduces the overhead on ALL your passive income.',
        cost: 250000,
        weeklyIncomeBonus: 0,
        level: 0,
        maxLevel: 5,
      },
    ],
};

/** Uniform per-level cost growth applied to every company upgrade. */
export const COMPANY_UPGRADE_COST_MULTIPLIER = 1.5;

/**
 * Starting weekly income per industry (was a flat 2000 for every type, which
 * made pricier companies strictly worse buys). Scales loosely with founding
 * cost so payback periods stay sane:
 *   factory    $50k  → $1,500/wk (~33 wk payback, cheap steady starter)
 *   ai         $90k  → $2,200/wk (~41 wk; big upgrade ceiling)
 *   restaurant $130k → $2,600/wk (~50 wk)
 *   realestate $200k → $3,200/wk (~63 wk)
 *   bank       $2M   → $4,000/wk (endgame; huge upgrade bonuses carry it)
 */
export const COMPANY_STARTING_INCOME: Record<string, number> = {
  factory: 1500,
  ai: 2200,
  restaurant: 2600,
  realestate: 3200,
  bank: 4000,
};
