/**
 * DiscordLogo — Discord's official "Clyde" mark as an inline SVG path.
 *
 * lucide-react-native ships no brand icons, and the invite surfaces were using
 * a generic `MessageCircle` instead, which reads as "chat" rather than "this
 * opens Discord". Recognition is the whole job of a join button, so the real
 * mark is drawn here once and shared.
 *
 * Single-colour by design (`color`, default white) so it can sit on the blurple
 * badge or be tinted for a muted link row. The viewBox is Discord's own
 * (127.14 × 96.36); `size` sets the WIDTH and the height follows the aspect
 * ratio, so callers never squash it.
 */
import React from 'react';
import Svg, { Path } from 'react-native-svg';

/** Discord brand blurple — the one colour the mark may sit on besides white. */
export const DISCORD_BLURPLE = '#5865F2';

interface DiscordLogoProps {
  /** Rendered width in px (already scaled by the caller). Height follows 96.36/127.14. */
  size?: number;
  color?: string;
}

const ASPECT = 96.36 / 127.14;

export default function DiscordLogo({ size = 24, color = '#FFFFFF' }: DiscordLogoProps) {
  const width = typeof size === 'number' && Number.isFinite(size) && size > 0 ? size : 24;
  return (
    <Svg width={width} height={width * ASPECT} viewBox="0 0 127.14 96.36">
      <Path
        fill={color}
        d="M107.7,8.07A105.15,105.15,0,0,0,81.47,0a72.06,72.06,0,0,0-3.36,6.83A97.68,97.68,0,0,0,49,6.83,72.37,72.37,0,0,0,45.64,0,105.89,105.89,0,0,0,19.39,8.09C2.79,32.65-1.71,56.6.54,80.21h0A105.73,105.73,0,0,0,32.71,96.36,77.7,77.7,0,0,0,39.6,85.25a68.42,68.42,0,0,1-10.85-5.18c.91-.66,1.8-1.34,2.66-2a75.57,75.57,0,0,0,64.32,0c.87.71,1.76,1.39,2.66,2a68.68,68.68,0,0,1-10.87,5.19,77,77,0,0,0,6.89,11.1A105.25,105.25,0,0,0,126.6,80.22h0C129.24,52.84,122.09,29.11,107.7,8.07ZM42.45,65.69C36.18,65.69,31,60,31,53s5-12.74,11.43-12.74S54,46,53.89,53,48.84,65.69,42.45,65.69Zm42.24,0C78.41,65.69,73.25,60,73.25,53s5-12.74,11.44-12.74S96.23,46,96.12,53,91.08,65.69,84.69,65.69Z"
      />
    </Svg>
  );
}
