...
export default function SeasonalEventModal({
 ...
 // Entry animation
 useEffect(() => {
 // BUGFIX: previously the two Animated.loop calls had no cleanup, so they
 // continued running after the modal closed — a memory leak that compounded
 // every time the user opened a seasonal event.
 ...
 }, [visible, fadeAnim, scaleAnim, sparkleAnim, pulseAnim]);
...
: `${countdown.weeks} ${countdown.weeks === 1 ? 'week': 'weeks'} left`}
 ...
 <View style={styles.rewardItem}>
 <View style={styles.rewardIconContainer}>
 <Package size={scale(20)} color="#F59E0B" strokeWidth={2.4} />
 </View>
 <View style={styles.rewardInfo}>
