...
function HighlightOverlay({ target, visible, message, position = 'bottom' }: HighlightOverlayProps) {
  ...
  useEffect(() => {
    if (visible && target) {
      try {
        // Use measureInWindow for more reliable measurements
        const measureMethod = target.measureInWindow || target.measure;

        if (typeof measureMethod !== 'function') {
          console.warn('Tutorial target does not have measure method');
          return;
        }

        measureMethod.call(target, (x: number, y: number, width: number, height: number) => {
          // Component might have unmounted during async measurement
          if (!visible) {
            console.log('Tutorial dismissed during measurement');
            return;
          }
          ...
          if (typeof x !== 'number' || typeof y !== 'number' ||
            ...
            console.warn('Invalid measurement values from tutorial target');
            return;
          }

          if (width <= 0 || height <= 0) {
            console.warn('Tutorial target has invalid dimensions:', { width, height });
            return;
          }
        ...
        console.error('Error measuring tutorial target:', error);
        // Continue without highlight - tutorial still works
      }
    }
