...
import { colors, accent, withAlpha } from '@/lib/config/theme';
  ...
  return <View style={styles.root}>
    ...
    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.options}>
      {PORTRAITS.map(p => <MotionPressable key={p.id} onPress={() => onChange(p.id)}
        ...
        style={[styles.option, { borderColor: value === p.id ? accent.info : colors.dark.border }]}>
        <Image source={PORTRAIT_ASSETS[p.id]} style={styles.image} accessibilityIgnoresInvertColors />
        {value === p.id && <View style={styles.check}><Check color={colors.dark.text} size={14} /></View>}
...
const styles = StyleSheet.create({
  ...
  name: { color: colors.dark.text, fontSize: fontScale(16), fontWeight: '600' },
  caption: { color: colors.dark.textSecondary, fontSize: fontScale(12), textAlign: 'center' },
  note: { color: colors.dark.textSecondary, fontSize: fontScale(12), lineHeight: fontScale(18) },
  arrow: { minWidth: 44, minHeight: 44, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.dark.surfaceInteractive, borderRadius: responsiveBorderRadius.lg },
  options: { gap: scale(8), paddingVertical: scale(4) },
  option: { borderWidth: 2, borderRadius: responsiveBorderRadius.xl, padding: scale(4), gap: scale(4), backgroundColor: colors.dark.surface },
