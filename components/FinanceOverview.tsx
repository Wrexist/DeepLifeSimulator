...
function FinanceOverview({ compact = false, onExpand }: FinanceOverviewProps) {
 ...
 // can freeze the UI thread after many weeks of accumulated assets).
 ...
 () => loans.reduce((sum, l) => sum + (l.remaining ?? l.principal ?? 0), 0),
 [loans]
 );
 const weeklyLoanPayments = useMemo(
 () => loans.reduce((sum, l) => sum + (l.weeklyPayment || 0), 0),
 [loans]
 );
 ...
 // Cheap scalar fingerprints so useMemo doesn't re-run on every render when
 // these arrays receive new object refs (which they do after every week tick).
 // Using length + a coarse total avoids walking the arrays repeatedly.
 const realEstateOwnedTotal = useMemo(
 () => (gameState.realEstate || []).reduce((s, re) => s + (re.owned ? (re.price || 0) : 0), 0),
 [gameState.realEstate]
 );
 const vehiclesOwnedTotal = useMemo(
 () => (gameState.vehicles || []).reduce(
 (s, v) => s + (v.owned ? (v.price * (v.condition / 100)) || 0 : 0),
 0,
 ),
 [gameState.vehicles]
 );
 const companiesAnnualIncome = useMemo(
 () => (gameState.companies || []).reduce((s, c) => s + (c.weeklyIncome * WEEKS_PER_YEAR || 0), 0),
 [gameState.companies]
 );
 ...
 total += realEstateOwnedTotal;
 total += vehiclesOwnedTotal;
 total += companiesAnnualIncome;
 total -= totalLoanDebt;
 return total;
 }, [
 ...
 <BarChart3 size={16} color={settings.darkMode ? '#60A5FA' : '#3B82F6'} />
