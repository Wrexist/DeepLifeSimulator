...
## 🚀 Releasing? Follow the runbook, not this file

**[`RELEASE_RUNBOOK.md`](./RELEASE_RUNBOOK.md) is the step-by-step procedure** —
version bump, secrets, preflight, build, every metadata field with its exact
value, screenshots, the Spanish localisation, IAP renames, submit. Do it top to
bottom.

This file stays as the broader **audit**: what is already handled in code versus
what only exists in the consoles. It is a reference, not a sequence. Where the
two disagree, the runbook is newer.
