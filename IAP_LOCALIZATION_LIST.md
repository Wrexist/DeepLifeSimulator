# Complete IAP Localization List for DeepLife Simulator

This document contains all In-App Purchases (IAPs) that need localization in App Store Connect (iOS) and Google Play Console (Android).

## Total IAPs: 25 Products + 2 Subscriptions = 27 Total

---

## 📦 CONSUMABLE PRODUCTS (15 items)

These products can be purchased multiple times and should NOT be restored on app reinstall.

### Gem Packs (Individual)
1. **Product ID:** `deeplife_gems_100`
   - **Name:** 100 Gems
   - **Description:** Small gem pack for quick purchases
   - **Price:** $0.99

2. **Product ID:** `deeplife_gems_500`
   - **Name:** 500 Gems
   - **Description:** Medium gem pack for regular players
   - **Price:** $4.99

3. **Product ID:** `deeplife_gems_1000`
   - **Name:** 1,000 Gems
   - **Description:** Large gem pack for active players
   - **Price:** $9.99
   - **Popular:** Yes

4. **Product ID:** `deeplife_gems_5000`
   - **Name:** 5,000 Gems
   - **Description:** Huge gem pack for dedicated players
   - **Price:** $19.99
   - **Best Value:** Yes

5. **Product ID:** `deeplife_gems_15000`
   - **Name:** 15,000 Gems
   - **Description:** Massive gem pack for power players
   - **Price:** $49.99

### Gem Bundles (Starter Packs)
6. **Product ID:** `deeplife_gems_starter`
   - **Name:** Starter Pack
   - **Description:** 1,000 Gems + 1 Youth Pill
   - **Price:** $9.99
   - **Original Price:** $14.98
   - **Savings:** 33%

7. **Product ID:** `deeplife_gems_premium`
   - **Name:** Premium Pack
   - **Description:** 3,500 Gems + 3 Youth Pills + Money Multiplier
   - **Price:** $24.99
   - **Popular:** Yes
   - **Original Price:** $44.97
   - **Savings:** 44%

8. **Product ID:** `deeplife_gems_ultimate`
   - **Name:** Ultimate Pack
   - **Description:** 12,000 Gems + 10 Youth Pills + All Permanent Upgrades
   - **Price:** $49.99
   - **Best Value:** Yes
   - **Original Price:** $199.90
   - **Savings:** 75%

9. **Product ID:** `deeplife_gems_mega`
   - **Name:** Mega Pack
   - **Description:** 40,000 Gems + Unlimited Youth Pills + Everything Unlocked
   - **Price:** $99.99
   - **Original Price:** $499.85
   - **Savings:** 80%

### Individual Items
10. **Product ID:** `deeplife_youth_pill_single`
    - **Name:** Youth Pill (Single)
    - **Description:** Reset age to 18 - One time use
    - **Price:** $4.99

11. **Product ID:** `deeplife_youth_pill_pack`
    - **Name:** Youth Pill Pack
    - **Description:** 5 Youth Pills - Great value
    - **Price:** $19.99
    - **Original Price:** $24.95
    - **Savings:** 20%

12. **Product ID:** `deeplife_money_boost`
    - **Name:** Money Boost
    - **Description:** Instant $1,000,000 cash injection
    - **Price:** $7.99

13. **Product ID:** `deeplife_skill_boost`
    - **Name:** Skill Boost
    - **Description:** All skills +50 levels instantly
    - **Price:** $12.99

14. **Product ID:** `revival_pack`
    - **Name:** Revival Pack
    - **Description:** Instantly revive your character when they die
    - **Price:** $2.99
    - **Popular:** Yes
    - **Features:**
      - Instant revival on death
      - Restore health to 50
      - Restore happiness to 50
      - Restore energy to 100
      - Continue your progress

### Repeatable Perk

**Product ID:** `deeplife_work_boost`
    - **Name:** Work Pay Boost
    - **Description:** +50% earnings on all jobs
    - **Price:** $1.99
    - **Note:** Consumable / repeatable — not restored on reinstall

---

## 🔒 NON-CONSUMABLE PRODUCTS (12 items)

These products are permanent and SHOULD be restored on app reinstall.

### Premium Features
15. **Product ID:** `deeplife_lifetime_premium`
    - **Name:** Lifetime Premium
    - **Description:** All future updates + exclusive content + no ads
    - **Price:** $79.99
    - **Features:**
      - All future updates
      - Exclusive content
      - No advertisements
      - Premium support
      - Early access to features

16. **Product ID:** `deeplife_remove_ads`
    - **Name:** Remove Ads
    - **Description:** Enjoy ad-free gaming experience
    - **Price:** $2.99
    - **Features:**
      - No more banner ads
      - No more interstitial ads
      - Faster game loading
      - Better performance

### Perks (Individual)
18. **Product ID:** `deeplife_mindset_perk`
    - **Name:** Mindset
    - **Description:** 50% faster promotions
    - **Price:** $1.99

19. **Product ID:** `deeplife_fast_learner`
    - **Name:** Fast Learner
    - **Description:** 50% faster education
    - **Price:** $1.99

20. **Product ID:** `deeplife_good_credit`
    - **Name:** Good Credit Score
    - **Description:** Higher bank interest rates
    - **Price:** $1.99

21. **Product ID:** `deeplife_unlock_all_perks`
    - **Name:** Unlock All Perks
    - **Description:** Includes all perks above
    - **Price:** $6.99
    - **Popular:** Yes
    - **Best Value:** Yes
    - **Original Price:** $7.96
    - **Savings:** 12%

### Bank Services (Computer Banking App Services)
22. **Product ID:** `deeplife_premium_credit_card`
    - **Name:** Premium Credit Card
    - **Description:** 10% cashback on all purchases
    - **Price:** $4.99
    - **Popular:** Yes
    - **Features:**
      - 10% cashback on all purchases
      - No annual fee
      - Travel insurance
      - 24/7 customer support

23. **Product ID:** `deeplife_financial_planning`
    - **Name:** Financial Planning
    - **Description:** 15% interest on bank savings
    - **Price:** $2.99
    - **Features:**
      - 15% interest on bank savings
      - Expert financial advice
      - Investment portfolio optimization
      - Monthly financial reports

24. **Product ID:** `deeplife_business_banking`
    - **Name:** Business Banking
    - **Description:** Company loans and upgrades
    - **Price:** $3.99
    - **Features:**
      - Company loans
      - Business account management
      - Company upgrade purchases
      - Dedicated business advisor

25. **Product ID:** `deeplife_private_banking`
    - **Name:** Private Banking
    - **Description:** VIP 3% APR loans
    - **Price:** $9.99
    - **Features:**
      - 3% APR loans (vs 5.5%)
      - Up to $200,000 loan limit
      - Personal wealth manager
      - Priority customer support

---

## 🔄 SUBSCRIPTION PRODUCTS (2 items)

26. **Product ID:** `deeplife_premium_monthly`
    - **Type:** Auto-Renewable Subscription (Monthly)
    - **Name:** Premium Monthly
    - **Description:** [To be configured - Monthly premium subscription]

27. **Product ID:** `deeplife_premium_yearly`
    - **Type:** Auto-Renewable Subscription (Yearly)
    - **Name:** Premium Yearly
    - **Description:** [To be configured - Yearly premium subscription]

---

## 📋 LOCALIZATION CHECKLIST

For each IAP product above, you need to provide localizations for:

### App Store Connect (iOS) Requirements:
- ✅ **Display Name** (localized)
- ✅ **Description** (localized)
- ✅ **Review Notes** (optional, but recommended)

### Google Play Console (Android) Requirements:
- ✅ **Title** (localized)
- ✅ **Description** (localized)
- ✅ **Short Description** (localized, max 80 characters)

### Top 20 Countries for App Store Localization:

Based on app revenue, user base, and market importance:

1. **United States** 🇺🇸
   - Language: English (US)
   - iOS Code: `en-US`
   - Android Code: `en-US`
   - Priority: ⭐⭐⭐⭐⭐ (Highest)

2. **China** 🇨🇳
   - Language: Chinese (Simplified)
   - iOS Code: `zh-Hans`
   - Android Code: `zh-CN`
   - Priority: ⭐⭐⭐⭐⭐ (Highest)

3. **Japan** 🇯🇵
   - Language: Japanese
   - iOS Code: `ja`
   - Android Code: `ja-JP`
   - Priority: ⭐⭐⭐⭐⭐ (Highest)

4. **United Kingdom** 🇬🇧
   - Language: English (UK)
   - iOS Code: `en-GB`
   - Android Code: `en-GB`
   - Priority: ⭐⭐⭐⭐

5. **Germany** 🇩🇪
   - Language: German
   - iOS Code: `de`
   - Android Code: `de-DE`
   - Priority: ⭐⭐⭐⭐

6. **South Korea** 🇰🇷
   - Language: Korean
   - iOS Code: `ko`
   - Android Code: `ko-KR`
   - Priority: ⭐⭐⭐⭐

7. **France** 🇫🇷
   - Language: French
   - iOS Code: `fr`
   - Android Code: `fr-FR`
   - Priority: ⭐⭐⭐⭐

8. **Canada** 🇨🇦
   - Language: English (Canada) / French (Canada)
   - iOS Code: `en-CA` / `fr-CA`
   - Android Code: `en-CA` / `fr-CA`
   - Priority: ⭐⭐⭐⭐

9. **Australia** 🇦🇺
   - Language: English (Australia)
   - iOS Code: `en-AU`
   - Android Code: `en-AU`
   - Priority: ⭐⭐⭐

10. **Brazil** 🇧🇷
    - Language: Portuguese (Brazil)
    - iOS Code: `pt-BR`
    - Android Code: `pt-BR`
    - Priority: ⭐⭐⭐⭐

11. **Italy** 🇮🇹
    - Language: Italian
    - iOS Code: `it`
    - Android Code: `it-IT`
    - Priority: ⭐⭐⭐

12. **Spain** 🇪🇸
    - Language: Spanish (Spain)
    - iOS Code: `es`
    - Android Code: `es-ES`
    - Priority: ⭐⭐⭐

13. **Mexico** 🇲🇽
    - Language: Spanish (Mexico)
    - iOS Code: `es-MX`
    - Android Code: `es-MX`
    - Priority: ⭐⭐⭐

14. **India** 🇮🇳
    - Language: English (India) / Hindi
    - iOS Code: `en-IN` / `hi`
    - Android Code: `en-IN` / `hi-IN`
    - Priority: ⭐⭐⭐⭐

15. **Russia** 🇷🇺
    - Language: Russian
    - iOS Code: `ru`
    - Android Code: `ru-RU`
    - Priority: ⭐⭐⭐

16. **Netherlands** 🇳🇱
    - Language: Dutch
    - iOS Code: `nl`
    - Android Code: `nl-NL`
    - Priority: ⭐⭐⭐

17. **Sweden** 🇸🇪
    - Language: Swedish
    - iOS Code: `sv`
    - Android Code: `sv-SE`
    - Priority: ⭐⭐⭐

18. **Saudi Arabia** 🇸🇦
    - Language: Arabic
    - iOS Code: `ar`
    - Android Code: `ar-SA`
    - Priority: ⭐⭐⭐

19. **Taiwan** 🇹🇼
    - Language: Chinese (Traditional)
    - iOS Code: `zh-Hant`
    - Android Code: `zh-TW`
    - Priority: ⭐⭐⭐

20. **Poland** 🇵🇱
    - Language: Polish
    - iOS Code: `pl`
    - Android Code: `pl-PL`
    - Priority: ⭐⭐⭐

**Additional High-Value Markets:**
- **Turkey** 🇹🇷 (Turkish - `tr`)
- **Argentina** 🇦🇷 (Spanish - `es-AR`)
- **Switzerland** 🇨🇭 (German/French/Italian - `de-CH`/`fr-CH`/`it-CH`)
- **Norway** 🇳🇴 (Norwegian - `no`)
- **Denmark** 🇩🇰 (Danish - `da`)

---

## 📝 NOTES

- **Consumable Products:** These should NOT be restored when the user reinstalls the app. Users can purchase them multiple times.
- **Non-Consumable Products:** These SHOULD be restored when the user reinstalls the app. They are permanent purchases.
- **Subscriptions:** These are auto-renewable subscriptions. Make sure to configure subscription groups and pricing tiers in both stores.
- **Product IDs:** Must match exactly between your code (`utils/iapConfig.ts`) and the store configurations.
- **Pricing:** Prices shown are in USD. Each store will handle currency conversion automatically based on the user's region.

---

## 🚀 QUICK REFERENCE: All Product IDs

```
deeplife_gems_100
deeplife_gems_500
deeplife_gems_1000
deeplife_gems_5000
deeplife_gems_15000
deeplife_gems_starter
deeplife_gems_premium
deeplife_gems_ultimate
deeplife_gems_mega
deeplife_youth_pill_single
deeplife_youth_pill_pack
deeplife_money_boost
deeplife_skill_boost
revival_pack
deeplife_lifetime_premium
deeplife_remove_ads
deeplife_work_boost
deeplife_mindset_perk
deeplife_fast_learner
deeplife_good_credit
deeplife_unlock_all_perks
deeplife_premium_credit_card
deeplife_financial_planning
deeplife_business_banking
deeplife_private_banking
deeplife_premium_monthly
deeplife_premium_yearly
```

---

**Last Updated:** Based on `utils/iapConfig.ts` configuration
**Total Products:** 25 IAPs + 2 Subscriptions = 27 Total Products

