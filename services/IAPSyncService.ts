...
class IAPSyncService {
  ...
  private async retryAction(action: IAPAction, attempt: number): Promise<void> {
    if (attempt >= this.maxRetries) {
      console.warn(`Action ${action.id} failed after ${this.maxRetries} attempts`);
      return;
    }
  ...
  private async savePendingActions(): Promise<void> {
    try {
      ...
      console.error('Failed to save IAP sync queue:', error);
    }
  }
  ...
  private async loadPendingActions(): Promise<void> {
    try {
      ...
      console.error('Failed to load IAP sync queue:', error);
      this.syncQueue = [];
    }
  }
  ...
  private notifyListeners(action: IAPAction): void {
    this.listeners.forEach(listener => {
      try {
        ...
        console.error('Error in IAP sync listener:', error);
      }
    });
  }
