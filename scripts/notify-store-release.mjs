...
// Explicit, though Node exposes Buffer as a global. A bare `Buffer` here was a
// no-undef ERROR that failed the Lint step of eas-update.yml for the whole repo:
// eslint.config.js declared Node globals for {js,jsx,ts,tsx} and
// scripts/**/*.{js,cjs}, and no glob covered .mjs. That gap is closed now (there
// is a scripts/**/*.mjs block), but the import stays — it matches the node:
// imports above and does not depend on lint config to be correct.
