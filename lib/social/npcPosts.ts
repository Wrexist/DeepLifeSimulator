...
const POST_TEMPLATES: Record<string, string[]> = {
 ...
 romantic: [
 "Love is in the air",
 "Spending time with someone special today",
...
const DATING_POST_TEMPLATES = [
 ...
 "Beautiful weather calls for outdoor activities!",
...
// ─────────────────────────────────────────────────────────────────────
// Pulse feed generation (v13+)
// ─────────────────────────────────────────────────────────────────────

/**
 * Generate NPC posts for the Pulse feed each weekly tick.
 *
 * Eligibility: relationships with score ≥ 30, not posted in the past 2 weeks.
 * 25% chance per eligible NPC to post (capped so the feed doesn't explode).
 *
 * Determinism: a per-NPC hash of (npcId, weeksLived) drives the 25% gate so
 * StrictMode re-renders don't generate different posts.
 */
export function generateNpcPostsForFeed(
  state: GameState,
  ...
  for (const p of state.socialPosts ?? []) {
    if (p.authorId && !p.isPlayerPost && typeof p.gameWeek === 'number') {
      const prev = lastPostByNpc[p.authorId] ?? -Infinity;
      if (p.gameWeek > prev) lastPostByNpc[p.authorId] = p.gameWeek;
    }
  }

  // Sort relationships by score desc so the highest-affinity NPCs post first
  // and the `maxPosts` cap favors people the player actually cares about.
  ...
  for (const rel of candidates) {
    ...
    const lastPost = lastPostByNpc[rel.id] ?? -Infinity;
    if (nextWeeksLived - lastPost < 2) continue;

    // Deterministic 25% gate from a stable hash
    const seed = `${rel.id}|${nextWeeksLived}`;
    let h = 0;
