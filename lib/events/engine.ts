 ...
 *
 * DETERMINISM FIX (2026-08-16 audit H7b). ~23 `generate()` bodies decided money
 * signs, fine amounts, percentages and which friend / pet / child / vehicle the
 * event was about with raw `Math.random()`. `generate()` runs inside the weekly
 * tick, i.e. inside a `setGameState` updater, so those draws had the same two
 * defects `pulseTick` and the notification ids were already fixed for:
 ...
 * here plus `seasonalEvents.ts` and ~10 test suites, and a signature change would
 * have reached into `contexts/`, which this change does not touch.
 */
const payloadRoll = (state: GameState, eventId: string): ((salt: string) => number) => {
 ...
 */
const pickSeeded = <T,>(list: readonly T[], roll: (salt: string) => number, salt: string): T =>
