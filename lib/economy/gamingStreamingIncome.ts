...
export function calcGamingStreamingIncome(
  ...
  // EXPLOIT FIX (M-2): decay by REAL elapsed weeks (currentWeek - uploadedAt),
  // not by array position. Position-based decay let a player keep income near the
  // cap forever by spamming fresh uploads to refill the top slots — each new item
  // pushed older ones down an index instead of letting them genuinely age out.
  // Falls back to index for legacy items that predate the uploadedAt field.
  const ageInWeeks = (uploadedAt: number | undefined, index: number): number => {
    if (typeof uploadedAt === 'number' && isFinite(uploadedAt)) {
      return Math.max(0, currentWeek - uploadedAt);
    }
    return index; // legacy fallback
  };

  // Calculate gaming earnings from videos with decay.
  ...
  if (gamingData.videos && gamingData.videos.length > 0) {
    ...
    gaming = recentVideos.reduce((sum, video, index) => {
      ...
      const decayFactor = Math.max(0, 1 - (ageInWeeks(video.uploadedAt, index) * 0.05));
  ...
  // Calculate streaming earnings from stream history with decay.
  ...
  if (gamingData.streamHistory && gamingData.streamHistory.length > 0) {
    ...
    streaming = recentStreams.reduce((sum, stream, index) => {
      ...
      const decayFactor = Math.max(0, 1 - (ageInWeeks(stream.uploadedAt, index) * 0.05));
