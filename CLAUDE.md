...
| `npm run type-check:tests` | `tsc --noEmit` over `tsconfig.tests.json` — **the test tree**, which `type-check` excludes. 30 errors outstanding. A type error in a test is usually a test asserting on a field that does not exist, i.e. asserting nothing |
