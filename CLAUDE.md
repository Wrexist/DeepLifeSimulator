...
- **v51 adds `metAt` on `Relationship`** — where and when somebody entered the
...
| `marketing/aso/` | Store metadata as data + `npm run check:aso`. The Apple listing is PUSHED from it, not pasted — `npm run asc:status` / `asc:release` / `asc:release:apply`, documented in `docs/ASC-AUTOMATION.md`. `docs/store-screenshot-design.md` covers the screenshot system |
