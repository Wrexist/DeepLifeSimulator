...
5b iOS privacy manifest · 5c iOS purpose strings · 5d QA tools not in store builds ·
6 IAP native module ·
...
`testflight` (extends `production`, adds `EXPO_PUBLIC_QA_TOOLS` so the paid selfie
route is testable on a release build — §5d fails the build if any other
store-reachable profile carries it) · `preview` (internal, devtools on) ·
