...
try {
 ...
 const allIds=await page.evaluate(()=>window.assetIds);
 const incremental=process.argv.includes('--destinations');
 const ids=incremental?allIds.filter(id=>id.startsWith('destination-')):allIds;
 const previous=incremental?JSON.parse(await fs.readFile(path.join(packRoot,'manifest.json'),'utf8')).assets:[];
 const assets=previous.filter(a=>!ids.includes(a.id));
 ...
 for(const id of ids){
  ...
  const budget=['city','room','home'].includes(id)||(id.startsWith('business-')||id.startsWith('destination-'))?20000:3000;
  ...
  if(id.startsWith('destination-')){const target=path.resolve(packRoot,'../../assets/images/scenes');await fs.mkdir(target,{recursive:true});await sharp(png).resize(720,540).webp({quality:85,alphaQuality:95}).toFile(path.join(target,id+'.webp'));}
